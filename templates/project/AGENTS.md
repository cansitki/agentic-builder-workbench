...
- Secrets stay in `[APPROVED SECRET STORE]`; values enter only through `secenv ask` and the native Workbench modal. Never request them in chat/terminal, commit, print, or log them. If the secure path is unavailable, stop and repair it.

...
var ptyHelperPath = "";
var MIDDLE_PAGES_HIDDEN_CLASS = "vin-middle-pages-hidden";
function areMiddlePagesHidden() {
  return typeof document !== "undefined" && !!document.body?.classList.contains(MIDDLE_PAGES_HIDDEN_CLASS);
}
function resizeWorkspace(app) {
...
function isRootLeaf(leaf) {
  const container = leaf?.view?.containerEl;
  return !!(container && typeof container.closest === "function" && container.closest(".workspace-split.mod-root"));
}
async function revealTerminalOutsideMiddle(app) {
  const terminalLeaves = app.workspace.getLeavesOfType(VIEW_TYPE);
  let leaf = terminalLeaves.find((candidate) => !isRootLeaf(candidate)) || null;
  if (!leaf) {
...
async function setMiddlePagesHidden(app, hidden, showNotice = true) {
  if (typeof document === "undefined" || !document.body) return false;
  if (hidden) {
    await revealTerminalOutsideMiddle(app);
  }
  document.body.classList.toggle(MIDDLE_PAGES_HIDDEN_CLASS, hidden);
  resizeWorkspace(app);
  setTimeout(() => resizeWorkspace(app), 50);
  if (showNotice) {
    new import_obsidian.Notice(hidden ? "Middle pages hidden" : "Middle pages shown");
  }
  return hidden;
}
...
async function openTerminalLink(app, rawUrl) {
  ...
  if (!url) return;
  const surfing = app?.plugins?.getPlugin?.("surfing");
  if (surfing && /^https?:\/\//i.test(url)) {
    try {
      const workspace = app.workspace;
      const existing = surfing.settings?.openInSameTab === true
        ? workspace.getLeavesOfType("surfing-view").sort((a, b) => (b.activeTime || 0) - (a.activeTime || 0))
        : [];
      ...
      console.warn("[Can Workbench] Could not open terminal link in Surfing", error);
    }
  }
...
var WikiLinkAutocomplete = class {
  constructor(app, terminal, writeToShell, containerEl) {
    this.active = false;
    this.query = "";
    this.results = [];
    ...
    this.terminal.attachCustomKeyEventHandler((e) => {
      if (e.type === "keydown" && (e.metaKey || e.ctrlKey && e.shiftKey) && String(e.key || "").toLowerCase() === "c" && this.terminal.hasSelection?.()) {
  ...
  copyTerminalSelection() {
    ...
    if (!text) return;
    try {
      const electronClipboard = typeof require === "function" ? require("electron")?.clipboard : null;
  ...
  filterResults() {
    ...
    this.filterTimer = setTimeout(() => {
      ...
      if (q.length === 0) {
        this.results = allEntries.sort((a, b) => b.mtime - a.mtime).slice(0, 10);
      } else {
        const prefix = [];
        const contains = [];
  ...
  async renderPreview() {
    ...
    const allTags = [.../* @__PURE__ */ new Set([...tags, ...frontmatterTags])];
...
var BookmarkManager = class {
  constructor(terminal, containerEl) {
    this.bookmarks = [];
    this.nextId = 1;
  ...
  addBookmark(label) {
    ...
    const bookmark = { id, marker, decoration, label: bookmarkLabel, timestamp: Date.now(), pipEl };
...
var TerminalSession = class {
  constructor(parent, id, cwd, app) {
    ...
    this.installNormalSelectionMode();
    ...
    const kickAutoHide = () => {
      ...
      autoHideTimer = setTimeout(dismissOverlay, 500);
    };
    ...
    this.containerEl.addEventListener("dragleave", (e) => {
      ...
      e.stopPropagation();
      dragCounter--;
      if (dragCounter <= 0) dismissOverlay();
    }, captureOpt);
  ...
  async saveDroppedImages(files) {
    ...
    const escaped = saved.map((p) => this.shellEscape(p)).join(" ");
    this.process.stdin?.write(escaped);
  ...
  installNormalSelectionMode() {
    const apply = () => {
      const selectionService = this.terminal?._core?._selectionService;
      if (!selectionService || selectionService.__canWorkbenchNormalSelection) return;
      const original = typeof selectionService.shouldForceSelection === "function"
        ? selectionService.shouldForceSelection.bind(selectionService)
        : null;
      if (!original) return;
      selectionService.shouldForceSelection = (event) => {
        if (event?.button === 0 && !event.altKey && !event.ctrlKey && !event.metaKey) return true;
        return original(event);
      };
      selectionService.__canWorkbenchNormalSelection = true;
    };
    apply();
    if (typeof requestAnimationFrame === "function") requestAnimationFrame(apply);
    setTimeout(apply, 250);
  }
  installVisibilityRenderGuard() {
    const selectionService = this.terminal?._core?._selectionService;
    const renderService = this.terminal?._core?._renderService;
    if (!selectionService || !renderService || selectionService.__canWorkbenchVisibilityGuard) return;
    const originalRefresh = typeof selectionService.refresh === "function"
      ? selectionService.refresh.bind(selectionService)
      : null;
    const originalHandleSelectionChanged = typeof renderService.handleSelectionChanged === "function"
      ? renderService.handleSelectionChanged.bind(renderService)
      : null;
    const originalRenderRows = typeof renderService._renderRows === "function"
      ? renderService._renderRows.bind(renderService)
      : null;
    if (!originalRefresh || !originalHandleSelectionChanged || !originalRenderRows) return;
    selectionService.refresh = (...args) => {
      if (!this.isVisible) {
        selectionService.__canWorkbenchDeferredRefresh = true;
        return;
      }
      selectionService.__canWorkbenchDeferredRefresh = false;
    ...
    renderService.handleSelectionChanged = (...args) => {
      ...
      return originalHandleSelectionChanged(...args);
    };
    ...
    selectionService.__canWorkbenchVisibilityGuard = true;
  }
  captureOutput() {
  ...
  show(skipFocus = false) {
    ...
    requestAnimationFrame(() => {
      this.fit();
      const selectionService = this.terminal?._core?._selectionService;
      if (selectionService?.__canWorkbenchDeferredRefresh) selectionService.refresh();
...
var _FullscreenManager = class _FullscreenManager {
  constructor(view) {
    this.overlay = null;
    this.tabBarEl = null;
    this.gridEl = null;
  ...
  enter(layout) {
    if (this.isOpen || _FullscreenManager.overlayOpen) return;
    if (this.view.sessions.length === 0) return;
    _FullscreenManager.overlayOpen = true;
    if (layout) this.layout = layout;
    const visibleSessions = this.view.getSessionsForActiveCategory();
    this.focusedSession = visibleSessions.includes(this.view.activeSession) ? this.view.activeSession : visibleSessions[0] ?? this.view.activeSession ?? this.view.sessions[0];
  ...
  exit() {
    ...
    this.tabBarEl = null;
    this.gridEl = null;
    _FullscreenManager.overlayOpen = false;
    ...
    if (this.resizeTimer) clearTimeout(this.resizeTimer);
    const visibleSessions = this.view.getSessionsForActiveCategory();
    const target = this.focusedSession && visibleSessions.includes(this.focusedSession) ? this.focusedSession : visibleSessions[0] || this.view.sessions[0] || null;
  ...
  renderFsTabs() {
    ...
    categoryArea.className = "vin-fs-category-area";
    this.view.renderCategoryControls(categoryArea, "fullscreen");
    this.tabBarEl.appendChild(categoryArea);
    const tabsArea = document.createElement("div");
    tabsArea.className = "vin-fs-tabs";
    const visibleTabs = this.view.getSessionsForActiveCategory();
    if (!visibleTabs.includes(this.focusedSession)) {
      this.focusedSession = visibleTabs.includes(this.view.activeSession) ? this.view.activeSession : visibleTabs[0] || null;
    }
    for (const session of visibleTabs) {
      ...
      tab.addEventListener("dragstart", (e) => {
        ...
        this.view.draggingSessionId = session.id;
        e.dataTransfer?.setData("text/plain", String(session.id));
        e.dataTransfer?.setData("application/x-vin-terminal-session", String(session.id));
        if (e.dataTransfer) e.dataTransfer.effectAllowed = "move";
        requestAnimationFrame(() => tab.classList.add("is-dragging"));
      });
      tab.addEventListener("dragend", () => this.view.clearDragState());
      tab.addEventListener("contextmenu", (e) => {
        ...
        if (this.view.sessions.length > 1) {
          menu.addItem(
            (item) => item.setTitle("Close").setIcon("x").onClick(() => {
              this.view.closeSession(session);
              this.savedPositions.delete(session);
              if (this.focusedSession === session) {
                const visible = this.view.getSessionsForActiveCategory();
                this.focusedSession = visible[visible.length - 1] || null;
              }
              this.renderFsTabs();
              this.rebuildPanes();
            })
    ...
    newTab.addEventListener("click", () => {
      const newest = this.view.createSession();
    ...
    (0, import_obsidian.setIcon)(exitBtn, "minimize-2");
    exitBtn.title = "Exit fullscreen";
    exitBtn.addEventListener("click", () => this.exit());
    controls.appendChild(exitBtn);
    this.tabBarEl.appendChild(controls);
  }
  startTabRename(tab, label, session) {
  ...
  rebuildPanes() {
    ...
    if (visibleSessions.length === 0) {
      ...
      empty.className = "vin-fullscreen-empty-category";
      empty.textContent = `No tabs in ${this.view.activeCategory || "Unsorted"}`;
  ...
  getVisibleSessions() {
    const all = this.view.getSessionsForActiveCategory();
...
_FullscreenManager.overlayOpen = false;
var FullscreenManager = _FullscreenManager;
var TerminalView = class extends import_obsidian.ItemView {
  ...
  getState() {
    return {
      ...
      categories: this.syncCategoriesFromShared(),
  ...
  async setState(state, result) {
    if (state?.sessions?.length > 0) {
      const savedCategories = Array.isArray(state.categories) ? state.categories : [];
      const sessionCategories = state.sessions.map((s) => s.category).filter(Boolean);
      this.categories = this.syncCategoriesFromShared([...savedCategories, ...sessionCategories]);
      this.activeCategory = this.resolveCategoryKey(state.activeCategory);
      ...
      for (const saved of state.sessions) {
        ...
        session.__cwTmuxName = saved.cwTmuxName || "";
        this.sessions.push(session);
        session.hide();
      }
      const visible = this.getSessionsForActiveCategory();
      const target = visible.find((s) => s.id === state.activeId) ?? visible[0] ?? null;
      this.applyActiveCategory(target, true);
      this.renderTabs();
    }
  ...
  async onOpen() {
    ...
    container.addEventListener("mousedown", (e) => {
      if (e.target.closest(".vin-terminal-tab-bar")) return;
    ...
    this.emptyStateEl = this.sessionsEl.createDiv({ cls: "vin-terminal-empty-category" });
    this.syncCategoriesFromShared();
    this.resizeObserver = new ResizeObserver(() => {
      if (this.resizeTimer) clearTimeout(this.resizeTimer);
  ...
  switchTo(session) {
    ...
    if (!this.isSessionVisibleInActiveCategory(session)) {
      this.activeCategory = session.category || "";
    }
  ...
  getWorkbenchPlugin() {
    return this.app?.plugins?.plugins?.["can-workbench"] || null;
  }
  getSharedCategories() {
    const gsd = this.getWorkbenchPlugin()?.settings?.gsd;
    if (!gsd || !Array.isArray(gsd.terminalCategories)) return null;
    return gsd.terminalCategories;
  }
  syncCategoriesFromShared(extraCategories = []) {
    const sessionCategories = this.sessions.map((s) => s?.category).filter(Boolean);
    const sharedCategories = this.getSharedCategories();
    const source = sharedCategories
      ? [...sharedCategories, ...sessionCategories, ...extraCategories]
      : [...this.categories, ...sessionCategories, ...extraCategories];
    this.categories = this.normalizeCategories(source);
    if (sharedCategories) {
      const sharedKey = this.normalizeCategories(sharedCategories).join("\0");
      const nextKey = this.categories.join("\0");
      if (sharedKey !== nextKey) this.saveSharedCategories();
    } else if (this.categories.length > 0) {
      this.saveSharedCategories();
    }
    return [...this.categories];
  }
  saveSharedCategories() {
    const plugin = this.getWorkbenchPlugin();
    if (!plugin) return;
    plugin.settings = plugin.settings || {};
    plugin.settings.gsd = plugin.settings.gsd || {};
    const categories = this.normalizeCategories(this.categories);
    plugin.settings.gsd.terminalCategories = categories;
    if (plugin.modules?.gsd?.settings) {
      plugin.modules.gsd.settings.terminalCategories = categories.slice();
    }
    if (plugin.settings.gsd.terminalDefaultCategory && !plugin.settings.gsd.terminalCategories.includes(plugin.settings.gsd.terminalDefaultCategory)) {
      plugin.settings.gsd.terminalDefaultCategory = "";
    }
    if (plugin.modules?.gsd?.settings && plugin.modules.gsd.settings.terminalDefaultCategory && !categories.includes(plugin.modules.gsd.settings.terminalDefaultCategory)) {
      plugin.modules.gsd.settings.terminalDefaultCategory = "";
    }
    plugin.saveSettings?.();
  }
  getSharedSessionCategoryMap() {
    const plugin = this.getWorkbenchPlugin();
    const gsd = plugin?.settings?.gsd;
    if (!gsd) return null;
    ...
    if (plugin.modules?.gsd?.settings) {
      plugin.modules.gsd.settings.terminalSessionCategories = gsd.terminalSessionCategories;
    }
    return gsd.terminalSessionCategories;
  }
  getSharedSessionCategorySourceMap() {
    const plugin = this.getWorkbenchPlugin();
    const gsd = plugin?.settings?.gsd;
    if (!gsd) return null;
    ...
    if (plugin.modules?.gsd?.settings) {
      plugin.modules.gsd.settings.terminalSessionCategorySources = gsd.terminalSessionCategorySources;
    }
    return gsd.terminalSessionCategorySources;
  }
  getSessionCategoryStorageKeyFor(workspaceName, tmuxName) {
    const name = String(tmuxName || "").trim();
    if (!name) return "";
    return `${String(workspaceName || "*").trim() || "*"}::${name}`;
  }
  getSessionTmuxName(session) {
    return String(session?.__cwTmuxName || session?.name || "").trim();
  }
  getSessionCategoryStorageKey(session) {
    return this.getSessionCategoryStorageKeyFor(session?.__cwWorkspace || "*", this.getSessionTmuxName(session));
  }
  getRememberedSessionCategory(workspaceName, tmuxName) {
    const map = this.getSharedSessionCategoryMap();
    if (!map) return null;
    const key = this.getSessionCategoryStorageKeyFor(workspaceName, tmuxName);
    if (!key || !Object.prototype.hasOwnProperty.call(map, key)) return null;
    return this.resolveAssignableCategory(map[key]);
  }
  saveRememberedSessionCategory(session, category) {
    const map = this.getSharedSessionCategoryMap();
    const sourceMap = this.getSharedSessionCategorySourceMap();
    const key = this.getSessionCategoryStorageKey(session);
    if (!map || !key) return;
    map[key] = this.resolveAssignableCategory(category);
    if (sourceMap) sourceMap[key] = "manual";
    this.getWorkbenchPlugin()?.saveSettings?.();
  }
  renameRememberedCategory(oldName, nextName) {
    const map = this.getSharedSessionCategoryMap();
    if (!map) return;
    let changed = false;
    ...
    if (changed) this.getWorkbenchPlugin()?.saveSettings?.();
  }
  clearRememberedCategory(name) {
    const map = this.getSharedSessionCategoryMap();
    if (!map) return;
    let changed = false;
    for (const key of Object.keys(map)) {
  ...
  updateRememberedSessionCategoryKey(oldWorkspace, oldTmuxName, nextWorkspace, nextTmuxName) {
    const map = this.getSharedSessionCategoryMap();
    const sourceMap = this.getSharedSessionCategorySourceMap();
    if (!map) return;
    const oldKey = this.getSessionCategoryStorageKeyFor(oldWorkspace, oldTmuxName);
    const nextKey = this.getSessionCategoryStorageKeyFor(nextWorkspace, nextTmuxName);
    if (!oldKey || !nextKey || oldKey === nextKey || !Object.prototype.hasOwnProperty.call(map, oldKey)) return;
    map[nextKey] = map[oldKey];
    delete map[oldKey];
    if (sourceMap && Object.prototype.hasOwnProperty.call(sourceMap, oldKey)) {
  ...
  normalizeCategories(categories) {
    ...
    for (const raw of categories) {
      const name = this.normalizeCategoryName(raw);
      if (!name || this.isReservedCategoryName(name)) continue;
  ...
  isReservedCategoryName(name) {
    return ["all", "uncategorized", "unsorted"].includes(String(name || "").trim().toLowerCase());
  }
  resolveCategoryKey(category) {
    ...
    if (!name) return "";
    return this.categories.includes(name) ? name : "all";
  }
  resolveAssignableCategory(category) {
    const name = this.normalizeCategoryName(category);
    if (!name || !this.categories.includes(name)) return "";
    return name;
  }
  defaultCategoryForNewSession() {
    return this.activeCategory && this.activeCategory !== "all" ? this.activeCategory : "";
  }
  getSessionsForCategory(category = this.activeCategory) {
    if (category === "all") return [...this.sessions];
    if (!category) return this.sessions.filter((s) => !s.category);
    return this.sessions.filter((s) => s.category === category);
  }
  getSessionsForActiveCategory() {
    return this.getSessionsForCategory(this.activeCategory);
  }
  isSessionVisibleInActiveCategory(session) {
    if (!session) return false;
    if (this.activeCategory === "all") return true;
    if (!this.activeCategory) return !session.category;
    return session.category === this.activeCategory;
  }
  applyActiveCategory(preferredSession = null, skipFocus = false) {
    const visible = this.getSessionsForActiveCategory();
    const target = preferredSession && visible.includes(preferredSession) ? preferredSession : visible.includes(this.activeSession) ? this.activeSession : visible[0] || null;
    for (const session of this.sessions) {
      if (session === target) {
        session.show(skipFocus || this.isRenaming);
      } else {
        session.hide();
      }
    }
    this.activeSession = target;
    this.updateEmptyState(visible.length === 0);
    return target;
  }
  updateEmptyState(forceVisible = null) {
    if (!this.emptyStateEl) return;
    const isVisible = forceVisible ?? this.getSessionsForActiveCategory().length === 0;
    this.emptyStateEl.textContent = this.activeCategory === "all" ? "No terminal tabs" : `No tabs in ${this.activeCategory || "Unsorted"}`;
    this.emptyStateEl.classList.toggle("is-visible", isVisible);
  }
  openCategory(category, preferredSession = null) {
    this.activeCategory = this.resolveCategoryKey(category);
    const target = this.applyActiveCategory(preferredSession);
    this.renderTabs();
    if (this.fullscreenManager?.isOpen) {
      this.fullscreenManager.focusedSession = target;
      this.fullscreenManager.renderFsTabs();
      this.fullscreenManager.rebuildPanes();
    }
    this.saveState();
  }
  createCategory(name, open = true) {
    const category = this.normalizeCategoryName(name);
    if (!category || this.isReservedCategoryName(category)) {
      new import_obsidian.Notice("Use a different category name");
      return null;
    }
    const existing = this.categories.find((c) => c.toLowerCase() === category.toLowerCase());
    const finalName = existing || category;
    if (!existing) {
      this.categories.push(finalName);
      this.saveSharedCategories();
    }
    if (open) {
      this.openCategory(finalName);
    } else {
      this.renderTabs();
      this.saveState();
    }
  ...
  openCategoryNameModal(currentName = "", onSubmit) {
    new TerminalCategoryNameModal(this.app, {
      title: currentName ? "Rename category" : "New category",
      initialValue: currentName,
      onSubmit: (name) => onSubmit(this.normalizeCategoryName(name))
    }).open();
  }
  openCategoryDeleteModal(name, onConfirm) {
    new TerminalCategoryConfirmModal(this.app, {
      title: "Delete category",
      message: `Tabs in "${name}" will move to Unsorted.`,
      confirmText: "Delete",
      onConfirm
    }).open();
  }
  addCategory() {
    this.openCategoryNameModal("", (name) => {
      if (name) this.createCategory(name, true);
    });
  }
  renameCategory(oldName) {
    this.openCategoryNameModal(oldName, (nextName) => {
      if (!nextName || nextName === oldName) return;
      if (this.isReservedCategoryName(nextName) || this.categories.some((c) => c !== oldName && c.toLowerCase() === nextName.toLowerCase())) {
        new import_obsidian.Notice("Use a unique category name");
        return;
      }
      this.categories = this.categories.map((c) => c === oldName ? nextName : c);
      for (const session of this.sessions) {
        if (session.category === oldName) session.category = nextName;
      }
      if (this.activeCategory === oldName) this.activeCategory = nextName;
      this.saveSharedCategories();
      this.renameRememberedCategory(oldName, nextName);
      this.renderTabs();
      if (this.fullscreenManager?.isOpen) {
        this.fullscreenManager.renderFsTabs();
        this.fullscreenManager.rebuildPanes();
  ...
  deleteCategory(name) {
    this.openCategoryDeleteModal(name, () => {
      this.categories = this.categories.filter((c) => c !== name);
      for (const session of this.sessions) {
        if (session.category === name) session.category = "";
      }
      if (this.activeCategory === name) this.activeCategory = "";
      this.saveSharedCategories();
      this.clearRememberedCategory(name);
      this.applyActiveCategory();
      this.renderTabs();
      if (this.fullscreenManager?.isOpen) {
        this.fullscreenManager.renderFsTabs();
  ...
  getDraggingSession(e) {
    const id = Number(e?.dataTransfer?.getData("text/plain") || this.draggingSessionId);
    return this.sessions.find((s) => s.id === id) || null;
  }
  clearDragState() {
    this.draggingSessionId = null;
    this.draggingCategory = null;
    document.querySelectorAll(".vin-terminal-tab, .vin-fs-tab, .vin-terminal-category").forEach((el) => {
  ...
  assignSessionCategory(session, category) {
    if (!session || category === "all") return;
    this.syncCategoriesFromShared([category]);
    const nextCategory = this.resolveAssignableCategory(category);
    session.category = nextCategory;
    this.saveRememberedSessionCategory(session, nextCategory);
    this.openCategory(nextCategory, session);
  }
  moveCategory(draggedCategory, targetCategory, placeAfter = false) {
    const from = this.categories.indexOf(draggedCategory);
    const target = this.categories.indexOf(targetCategory);
    if (from < 0 || target < 0 || from === target) return;
    const [moved] = this.categories.splice(from, 1);
    let to = target + (placeAfter ? 1 : 0);
    if (from < to) to -= 1;
    this.categories.splice(Math.max(0, Math.min(to, this.categories.length)), 0, moved);
    this.saveSharedCategories();
    this.renderTabs();
    if (this.fullscreenManager?.isOpen) {
      this.fullscreenManager.renderFsTabs();
  ...
  showSessionCategoryMenu(session, event) {
    event?.preventDefault();
    event?.stopPropagation();
    const menu = new import_obsidian.Menu();
    const addChoice = (label, category) => {
      menu.addItem((item) => {
        item.setTitle(`${session.category === category ? "✓ " : ""}${label}`).setIcon(session.category === category ? "check" : "folder").onClick(() => this.assignSessionCategory(session, category));
      });
    };
    addChoice("Unsorted", "");
    ...
    if (!this.categories.some((category) => String(category).toLowerCase() === "system")) {
      addChoice("System", "System");
    }
    menu.addSeparator();
    menu.addItem((item) => {
      item.setTitle("New category").setIcon("plus").onClick(() => {
        this.openCategoryNameModal("", (name) => {
          const created = this.createCategory(name, false);
          if (created) this.assignSessionCategory(session, created);
        });
      });
    });
  ...
  renderCategoryControls(parent, variant = "normal") {
    const list = document.createElement("div");
    list.className = variant === "fullscreen" ? "vin-fs-category-list" : "vin-terminal-categories-scroll";
    parent.appendChild(list);
    const addChip = (label, category, count) => {
      const chip = document.createElement("button");
      chip.type = "button";
      chip.className = "vin-terminal-category";
      if (this.activeCategory === category) chip.classList.add("is-active");
      if (String(category).toLowerCase() === "system") chip.classList.add("is-system");
      chip.dataset.category = category || "uncategorized";
      const name = document.createElement("span");
      name.className = "vin-terminal-category-name";
      name.textContent = label;
      chip.appendChild(name);
      const badge = document.createElement("span");
      badge.className = "vin-terminal-category-count";
      badge.textContent = String(count);
      chip.appendChild(badge);
      chip.addEventListener("click", () => this.openCategory(category));
      if (category && category !== "all") {
        chip.draggable = true;
        chip.addEventListener("dragstart", (e) => {
          this.draggingCategory = category;
          e.dataTransfer?.setData("application/x-vin-terminal-category", category);
          if (e.dataTransfer) e.dataTransfer.effectAllowed = "move";
          requestAnimationFrame(() => chip.classList.add("is-dragging"));
        });
        chip.addEventListener("dragend", () => this.clearDragState());
      }
      if (category !== "all") {
        chip.addEventListener("dragover", (e) => {
          if (this.draggingCategory && category) {
            ...
            const after = variant === "fullscreen" ? e.clientY > rect.top + rect.height / 2 : e.clientX > rect.left + rect.width / 2;
            chip.classList.toggle("is-drop-before", !after);
            chip.classList.toggle("is-drop-after", after);
            return;
          }
          if (!this.getDraggingSession(e)) return;
          e.preventDefault();
          if (e.dataTransfer) e.dataTransfer.dropEffect = "move";
        ...
        chip.addEventListener("dragleave", () => {
          ...
          chip.classList.remove("is-drop-before");
        ...
        chip.addEventListener("drop", (e) => {
          ...
          if (this.draggingCategory && category) {
            const rect = chip.getBoundingClientRect();
            const after = variant === "fullscreen" ? e.clientY > rect.top + rect.height / 2 : e.clientX > rect.left + rect.width / 2;
            this.moveCategory(this.draggingCategory, category, after);
            this.clearDragState();
            return;
          }
          this.assignSessionCategory(this.getDraggingSession(e), category);
          this.clearDragState();
        });
      }
      if (category && category !== "all") {
        chip.addEventListener("contextmenu", (e) => {
          e.preventDefault();
          const menu = new import_obsidian.Menu();
          menu.addItem((item) => item.setTitle("Rename").setIcon("pencil").onClick(() => this.renameCategory(category)));
          menu.addItem((item) => item.setTitle("Remove").setIcon("trash").onClick(() => this.deleteCategory(category)));
          menu.showAtMouseEvent(e);
        });
      }
      list.appendChild(chip);
    };
    addChip("All", "all", this.sessions.length);
    addChip("Unsorted", "", this.getSessionsForCategory("").length);
    for (const category of this.categories) {
      addChip(category, category, this.getSessionsForCategory(category).length);
    }
    const addButton = document.createElement("button");
    addButton.type = "button";
    addButton.className = "vin-terminal-category-add";
    addButton.title = "New category";
    addButton.textContent = "+";
    addButton.addEventListener("click", () => this.addCategory());
    parent.appendChild(addButton);
  }
  moveSession(draggedSession, targetSession, placeAfter = false) {
    if (!draggedSession || !targetSession || draggedSession === targetSession) return;
    const from = this.sessions.indexOf(draggedSession);
    const target = this.sessions.indexOf(targetSession);
    if (from < 0 || target < 0) return;
    const [moved] = this.sessions.splice(from, 1);
    let to = target + (placeAfter ? 1 : 0);
    if (from < to) to -= 1;
    this.sessions.splice(Math.max(0, Math.min(to, this.sessions.length)), 0, moved);
    this.renderTabs();
    this.saveState();
  }
  renderTabs() {
    if (this.isRenaming) return;
    this.syncCategoriesFromShared();
    this.applyActiveCategory(null, true);
    this.tabBarEl.empty();
    const categoryRow = this.tabBarEl.createDiv({ cls: "vin-terminal-category-row" });
    this.renderCategoryControls(categoryRow, "normal");
    const tabRow = this.tabBarEl.createDiv({ cls: "vin-terminal-tab-row" });
    const tabsArea = tabRow.createDiv({ cls: "vin-terminal-tabs-scroll" });
    this.getSessionsForActiveCategory().forEach((session) => {
      ...
      const categoryButton = tab.createEl("button", {
        cls: `vin-terminal-tab-category${session.category ? "" : " is-unsorted"}${String(session.category).toLowerCase() === "system" ? " is-system" : ""}`,
        text: session.category || "Unsorted",
        attr: { title: "Change category" }
      });
      categoryButton.addEventListener("click", (e) => this.showSessionCategoryMenu(session, e));
      categoryButton.addEventListener("mousedown", (e) => e.stopPropagation());
      categoryButton.addEventListener("dragstart", (e) => e.preventDefault());
      ...
      tab.addEventListener("dragover", (e) => {
        const dragged = this.sessions.find((s) => s.id === this.draggingSessionId);
        if (!dragged || dragged === session) return;
        e.preventDefault();
        if (e.dataTransfer) e.dataTransfer.dropEffect = "move";
        const rect = tab.getBoundingClientRect();
        const after = e.clientX > rect.left + rect.width / 2;
        tab.classList.toggle("is-drop-before", !after);
        tab.classList.toggle("is-drop-after", after);
      });
      tab.addEventListener("dragleave", () => {
        tab.classList.remove("is-drop-before");
        tab.classList.remove("is-drop-after");
      });
      tab.addEventListener("drop", (e) => {
        e.preventDefault();
        tab.classList.remove("is-drop-before");
        tab.classList.remove("is-drop-after");
        const draggedId = Number(e.dataTransfer?.getData("text/plain") || this.draggingSessionId);
        const dragged = this.sessions.find((s) => s.id === draggedId);
        const rect = tab.getBoundingClientRect();
        this.moveSession(dragged, session, e.clientX > rect.left + rect.width / 2);
      });
      ...
      tab.addEventListener("contextmenu", (e) => {
        ...
        menu.addItem(
          (item) => item.setTitle("Change category").setIcon("folder").onClick(() => {
            this.showSessionCategoryMenu(session, e);
          })
        );
        menu.addItem(
          (item) => item.setTitle("Close").setIcon("x").onClick(() => {
    ...
    const controls = tabRow.createDiv({ cls: "vin-terminal-tab-controls" });
    const middleHidden = areMiddlePagesHidden();
    const middlePagesBtn = controls.createDiv({ cls: `vin-terminal-tab-middle-pages${middleHidden ? " is-hidden" : ""}` });
    (0, import_obsidian.setIcon)(middlePagesBtn, middleHidden ? "panel-left-open" : "panel-left-close");
    middlePagesBtn.title = middleHidden ? "Show middle pages" : "Hide middle pages";
    middlePagesBtn.addEventListener("click", async () => {
      await toggleMiddlePages(this.app);
      this.renderTabs();
    });
    const fsBtn = controls.createDiv({ cls: "vin-terminal-tab-fullscreen" });
    (0, import_obsidian.setIcon)(fsBtn, "expand");
...
var TerminalCategoryNameModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    contentEl.addClass("vin-category-modal");
    contentEl.createEl("h3", { text: this.options.title });
    const input = contentEl.createEl("input", {
      cls: "vin-category-modal-input",
      attr: { type: "text", spellcheck: "false", placeholder: "Category name" }
    });
    input.value = this.options.initialValue || "";
    const actions = contentEl.createDiv({ cls: "vin-category-modal-actions" });
    const cancelBtn = actions.createEl("button", { text: "Cancel", cls: "vin-category-modal-btn" });
    const saveBtn = actions.createEl("button", { text: "Save", cls: "vin-category-modal-btn mod-cta" });
    const submit = () => {
      const name = input.value.trim();
      if (!name) {
        new import_obsidian.Notice("Category name is required");
        input.focus();
        return;
      }
      this.close();
...
var TerminalCategoryConfirmModal = class extends import_obsidian.Modal {
  constructor(app, options) {
    super(app);
    this.options = options;
  }
  onOpen() {
    ...
    contentEl.addClass("vin-category-modal");
    contentEl.createEl("h3", { text: this.options.title });
    contentEl.createEl("p", { text: this.options.message, cls: "setting-item-description" });
    const actions = contentEl.createDiv({ cls: "vin-category-modal-actions" });
    const cancelBtn = actions.createEl("button", { text: "Cancel", cls: "vin-category-modal-btn" });
    const confirmBtn = actions.createEl("button", { text: this.options.confirmText || "Confirm", cls: "vin-category-modal-btn mod-warning" });
    cancelBtn.addEventListener("click", () => this.close());
...
var ShortcutsModal = class extends import_obsidian.Modal {
  onOpen() {
    const { contentEl } = this;
...
var OutputCaptureModal = class extends import_obsidian.SuggestModal {
  constructor(app, capturedText) {
    super(app);
...
var TerminalPlugin = class extends import_obsidian.Plugin {
  async onload() {
    ...
    fs.writeFileSync(helperPath, PTY_HELPER_PY, { mode: 493 });
    ...
    this.addCommand({
      id: "close-middle-pages",
      name: "Hide Middle Pages",
      callback: () => setMiddlePagesHidden(this.app, true)
    });
    this.addCommand({
      id: "show-middle-pages",
      name: "Show Middle Pages",
      callback: () => setMiddlePagesHidden(this.app, false)
    });
    this.addCommand({
      id: "toggle-middle-pages",
      name: "Toggle Middle Pages",
      callback: () => toggleMiddlePages(this.app)
    });
    this.addCommand({
  ...
  async onunload() {
    await setMiddlePagesHidden(this.app, false, false);

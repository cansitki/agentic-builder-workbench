...
var require_addon_serialize = __commonJS({
  "node_modules/@xterm/addon-serialize/lib/addon-serialize.js"(exports, module2) {
    !function(e, t) {
      "object" == typeof exports && "object" == typeof module2 ? module2.exports = t() : "function" == typeof define && define.amd ? define([], t) : "object" == typeof exports ? exports.SerializeAddon = t() : e.SerializeAddon = t();
    }(exports, () => (() => {
      "use strict";
      var e = { 930: (e2, t2, s2) => {
        Object.defineProperty(t2, "__esModule", { value: true }), t2.ColorContrastCache = void 0;
...
// src/secrets.ts
var SECRETS_FILE = "secrets.enc";
var OBFUSCATION_KEY = "gsd-control-vault-2024";
var cache = null;
var cachedDir = "";
...
function secretsPath(pluginDir) {
  const path = require("path");
  return path.join(pluginDir, SECRETS_FILE);
}
function loadSecrets(pluginDir) {
  if (cache && cachedDir === pluginDir) return cache;
  const fs = require("fs");
  const filePath = secretsPath(pluginDir);
  try {
    const content = fs.readFileSync(filePath, "utf-8");
    const plain = deobfuscate(content);
    cache = JSON.parse(plain);
    cachedDir = pluginDir;
    console.log("[Workbench Secrets] Loaded from disk:", filePath);
    return cache;
  } catch {
    console.log("[Workbench Secrets] Starting fresh (no existing file or read error)");
    cache = {};
    cachedDir = pluginDir;
    return cache;
  }
}
function saveSecrets(pluginDir) {
  if (!cache) return;
  const fs = require("fs");
  const filePath = secretsPath(pluginDir);
  try {
    ...
    console.error("[Workbench Secrets] Failed to save:", e);
  }
}
function setSecret(pluginDir, key, value) {
  const secrets = loadSecrets(pluginDir);
  secrets[key] = value;
  saveSecrets(pluginDir);
}
function getSecret(pluginDir, key) {
  const secrets = loadSecrets(pluginDir);
  return secrets[key] || "";
}
function removeSecret(pluginDir, key) {
  const secrets = loadSecrets(pluginDir);
  delete secrets[key];
  saveSecrets(pluginDir);
}
var SECRET_KEYS = {
  sshKey: (workspaceName) => `ssh_key_${workspaceName}`
};

// src/ssh.ts
function workspaceSshHost(coderName, coderUser) {
  return `main.${coderName}.${coderUser}.coder`;
}
function shellPath() {
  const os = require("os");
  ...
  for (const entry of existingPath.split(":")) {
    if (entry && !pathSet.has(entry)) {
...
function execInWorkspace(host, command, keyPath) {
    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        if (error) {
          reject(
            new Error(
              `ssh exec failed for host=${host}: ${error.message}` + (stderr ? `
stderr: ${stderr.trim()}` : "")
            )
          );
        } else {
...
function escapeShellSingleQuote(input) {
  return input.replace(/'/g, "'\\''");
}

// src/SettingsTab.ts
var ProjectPickerModal = class extends import_obsidian.Modal {
  constructor(app, discovered, alreadyConfigured, onConfirm) {
    super(app);
    this.discovered = discovered;
    this.alreadyConfigured = alreadyConfigured;
    this.onConfirm = onConfirm;
    this.checked = new Map(
      discovered.map((p) => [p, !alreadyConfigured.has(p)])
    );
  }
  onOpen() {
    const { contentEl } = this;
    contentEl.empty();
    contentEl.createEl("h3", { text: "Discovered Workbench Projects" });
    contentEl.createEl("p", {
      text: "Select the projects you want to add to this workspace. Already-configured projects are shown but unchecked.",
      cls: "setting-item-description"
    });
    const listEl = contentEl.createDiv({ cls: "gsd-picker-list" });
    for (const proj of this.discovered) {
      const row = listEl.createDiv({ cls: "gsd-picker-row" });
      const label = row.createEl("label", { cls: "gsd-picker-label" });
      const cb = label.createEl("input", { type: "checkbox" });
      cb.checked = this.checked.get(proj) ?? false;
      cb.addEventListener("change", () => {
        this.checked.set(proj, cb.checked);
      });
      label.createSpan({ text: "  " + proj });
      if (this.alreadyConfigured.has(proj)) {
        label.createEl("span", {
          text: " (already added)",
          cls: "gsd-picker-badge"
        });
      }
    }
    const btnRow = contentEl.createDiv({ cls: "gsd-picker-buttons" });
    const addBtn = btnRow.createEl("button", { text: "Add Selected", cls: "mod-cta" });
    addBtn.addEventListener("click", () => {
      const selected = this.discovered.filter((p) => this.checked.get(p));
...
var GSDSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Can Workbench \u2014 Settings" });
    const infoBox = containerEl.createDiv({ cls: "gsd-settings-info" });
    infoBox.createEl("p", {
      text: "The plugin connects over SSH using Coder workspace aliases. Make sure `coder config-ssh` has been run at least once on this Mac \u2014 it writes the SSH aliases to ~/.ssh/config so the plugin can reach your workspaces."
    });
    new import_obsidian.Setting(containerEl).setName("Coder username").setDesc(
      "Your Coder username. The SSH host for each workspace is built as: main.<workspace>.<coderUser>.coder"
    ).addText(
      (text) => text.setPlaceholder("e.g. jsmith").setValue(this.plugin.settings.coderUser).onChange(async (value) => {
        this.plugin.settings.coderUser = value.trim();
    ...
    workspaces.forEach((ws, wsIdx) => {
      this.renderWorkspace(containerEl, ws, wsIdx);
    });
    new import_obsidian.Setting(containerEl).addButton(
      (btn) => btn.setButtonText("+ Add workspace").setCta().onClick(async () => {
        const newWs = {
          coderName: "",
          displayName: "New Workspace",
          projects: []
        };
        this.plugin.settings.workspaces.push(newWs);
        await this.plugin.saveSettings();
        this.display();
      })
    );
    ...
    style.textContent = `
      ...
      .gsd-settings-info p { margin: 0; }
      .gsd-ssh-host-preview {
      ...
      .gsd-workspace-section {
        border: 1px solid var(--background-modifier-border);
        border-radius: 6px;
        padding: 12px 16px;
        margin-bottom: 16px;
      }
      .gsd-project-row { margin-bottom: 4px; }
      .gsd-picker-list { margin: 12px 0; max-height: 320px; overflow-y: auto; }
      .gsd-picker-row { margin-bottom: 6px; }
      .gsd-picker-label { display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: 14px; }
      .gsd-picker-label input[type=checkbox] { width: 16px; height: 16px; cursor: pointer; }
      .gsd-picker-badge { font-size: 11px; color: var(--text-muted); font-style: italic; }
      .gsd-picker-buttons { display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px; }
    `;
  }
  renderWorkspace(containerEl, ws, wsIdx) {
    const wsSection = containerEl.createDiv({ cls: "gsd-workspace-section" });
    wsSection.createEl("h4", {
      text: ws.displayName || `Workspace ${wsIdx + 1}`
    });
    new import_obsidian.Setting(wsSection).setName("Display name").addText(
      (text) => text.setPlaceholder("My Workspace").setValue(ws.displayName).onChange(async (value) => {
        ws.displayName = value;
        await this.plugin.saveSettings();
      })
    );
    const coderUser = this.plugin.settings.coderUser;
    const previewHost = ws.coderName && coderUser ? `main.${ws.coderName}.${coderUser}.coder` : "main.<workspace>.<coderUser>.coder";
    const wsSetting = new import_obsidian.Setting(wsSection).setName("Coder workspace name").setDesc("The workspace slug from the Coder dashboard URL.");
    const previewEl = wsSection.createDiv();
    previewEl.createEl("span", { text: "SSH host: ", cls: "setting-item-description" });
    previewEl.createEl("code", { text: previewHost, cls: "gsd-ssh-host-preview" });
    previewEl.style.marginTop = "-8px";
    previewEl.style.marginBottom = "8px";
    wsSetting.addText(
      (text) => text.setPlaceholder("my-workspace").setValue(ws.coderName).onChange(async (value) => {
        ws.coderName = value.trim();
        await this.plugin.saveSettings();
        const newHost = ws.coderName && coderUser ? `main.${ws.coderName}.${coderUser}.coder` : "main.<workspace>.<coderUser>.coder";
        const codeEl = previewEl.querySelector("code");
        if (codeEl) codeEl.textContent = newHost;
      })
    );
    const pluginDir = this.plugin.getPluginDir();
    const sshKeySetting = new import_obsidian.Setting(wsSection).setName("SSH Private Key").setDesc(
      ws.hasKey ? "\u2705 Key stored (XOR-obfuscated). Paste a new key to replace, or remove below." : "Optional. Paste your SSH private key here. Stored XOR-obfuscated in the plugin directory."
    );
    sshKeySetting.addTextArea((ta) => {
      ta.setPlaceholder("-----BEGIN OPENSSH PRIVATE KEY-----\n...\n-----END OPENSSH PRIVATE KEY-----");
      ta.inputEl.rows = 4;
      ...
      ta.onChange(async (value) => {
        if (value.trim()) {
          setSecret(pluginDir, SECRET_KEYS.sshKey(ws.coderName), value.trim());
          ws.hasKey = true;
          await this.plugin.saveSettings();
          new import_obsidian.Notice("SSH key saved for " + ws.displayName);
        }
      });
    });
    const actionSetting = new import_obsidian.Setting(wsSection);
    if (ws.hasKey) {
      actionSetting.addButton(
        (btn) => btn.setButtonText("Remove SSH Key").setWarning().onClick(async () => {
          removeSecret(pluginDir, SECRET_KEYS.sshKey(ws.coderName));
          ws.hasKey = false;
          await this.plugin.saveSettings();
          new import_obsidian.Notice("SSH key removed for " + ws.displayName);
          this.display();
        })
      );
    }
    actionSetting.addButton(
      (btn) => btn.setButtonText("Test Connection").onClick(async () => {
        const cu = this.plugin.settings.coderUser;
        if (!cu || !ws.coderName) {
          new import_obsidian.Notice("Set Coder username and workspace name first.");
          return;
        }
        const host = workspaceSshHost(ws.coderName, cu);
        const { execFile } = require("child_process");
        const sshArgs = [];
        let tempKeyPath = "";
        if (ws.hasKey) {
          const fs = require("fs");
          const keyData = getSecret(pluginDir, SECRET_KEYS.sshKey(ws.coderName));
          if (keyData) {
            tempKeyPath = `/tmp/gsd-ssh-test-${ws.coderName}`;
        ...
        new import_obsidian.Notice("Testing connection to " + host + "...");
        execFile("/usr/bin/ssh", sshArgs, { timeout: 1e4, env: { ...process.env, PATH: shellPath() } }, (error, stdout) => {
          if (tempKeyPath) {
            try {
          ...
          if (error) {
            new import_obsidian.Notice("\u274C Connection failed: " + (error.message || String(error)), 8e3);
          } else if (stdout.trim() === "ok") {
            new import_obsidian.Notice("\u2705 Connected to " + host);
          } else {
            new import_obsidian.Notice("\u26A0\uFE0F Unexpected response: " + stdout.trim(), 6e3);
          }
        });
      })
    );
    actionSetting.addButton(
      (btn) => btn.setButtonText("Discover Projects").onClick(async () => {
        const cu = this.plugin.settings.coderUser;
        if (!cu || !ws.coderName) {
          new import_obsidian.Notice("Set Coder username and workspace name first.");
          return;
        ...
        const host = workspaceSshHost(ws.coderName, cu);
        new import_obsidian.Notice("Scanning " + host + " for workspace projects...");
        let keyPath;
        let tempKeyPath = "";
        if (ws.hasKey) {
          const fs = require("fs");
          const keyData = getSecret(pluginDir, SECRET_KEYS.sshKey(ws.coderName));
          if (keyData) {
            tempKeyPath = `/tmp/gsd-discover-${ws.coderName}`;
            fs.writeFileSync(tempKeyPath, keyData + "\n", { mode: 384 });
            keyPath = tempKeyPath;
          }
        }
        try {
          const cmd = [
            'find -L "$HOME" -maxdepth 5 -name .gsd -type d',
            '! -path "$HOME/.gsd"',
            // exclude legacy global config
            "! -path '*/.gsd/worktrees/*'",
            // exclude worktree nested .gsd dirs
            "! -path '*/node_modules/*'",
            "! -path '*/.git/*'",
            "2>/dev/null",
            "| sed 's|/.gsd$||'",
          ...
          const output = await execInWorkspace(host, cmd, keyPath);
          if (tempKeyPath) {
            try {
              require("fs").unlinkSync(tempKeyPath);
            } catch {
          ...
          if (!output) {
            new import_obsidian.Notice("No workspace projects found in " + host + ". Make sure the project metadata exists.");
            return;
          }
          const discovered = output.split("\n").map((l) => l.trim()).filter(Boolean);
          const alreadyConfigured = new Set(ws.projects.map((p) => p.path));
          new ProjectPickerModal(
            ...
            async (selected) => {
              ...
              for (const relPath of selected) {
                if (!alreadyConfigured.has(relPath)) {
                  const displayName = relPath.split("/").pop() || relPath;
                  ws.projects.push({ path: relPath, displayName });
                  added++;
                }
              }
              await this.plugin.saveSettings();
              if (added === 0) {
                new import_obsidian.Notice("No new projects added.");
              } else {
                new import_obsidian.Notice(`\u2705 Added ${added} project(s).`);
              }
              this.display();
            }
          ).open();
          ...
          new import_obsidian.Notice("\u274C Discovery failed: " + msg, 8e3);
        }
      })
    );
    wsSection.createEl("h5", { text: "Projects" });
    if (ws.projects.length === 0) {
      wsSection.createEl("p", {
        text: 'No projects yet. Click "Discover Projects" to auto-detect, or add manually.',
        cls: "setting-item-description"
      });
    }
    ws.projects.forEach((proj, projIdx) => {
      this.renderProject(wsSection, ws, wsIdx, proj, projIdx);
    });
    new import_obsidian.Setting(wsSection).addButton(
      (btn) => btn.setButtonText("+ Add project manually").onClick(async () => {
        const newProj = {
          path: "",
      ...
      (btn) => btn.setButtonText("Remove workspace").setWarning().onClick(async () => {
        this.plugin.settings.workspaces.splice(wsIdx, 1);
        await this.plugin.saveSettings();
        this.display();
      })
    );
  }
  renderProject(wsSection, ws, _wsIdx, proj, projIdx) {
    const projDiv = wsSection.createDiv({ cls: "gsd-project-row" });
    new import_obsidian.Setting(projDiv).setName(`Project ${projIdx + 1}`).addText(
      (text) => text.setPlaceholder("Display name").setValue(proj.displayName).onChange(async (value) => {
        proj.displayName = value;
        await this.plugin.saveSettings();
      })
    ).addText(
      (text) => text.setPlaceholder("Path relative to ~ (e.g. app)").setValue(proj.path).onChange(async (value) => {
        proj.path = value.trim();
        await this.plugin.saveSettings();
      })
    ).addButton(
      (btn) => btn.setButtonText("Remove").setWarning().onClick(async () => {
        ws.projects.splice(projIdx, 1);
        await this.plugin.saveSettings();
        this.display();
      })
    );
...
var DEFAULT_SETTINGS = {
  ...
  workspaces: [],
  terminalCategories: [],
  terminalDefaultCategory: "",
  terminalSessionCategories: {}
};
...
var FALLBACK_LIGHT_THEME = {
  background: "#ffffff",
  foreground: "#1a1a1a",
  cursor: "#705dcf",
  selectionBackground: "rgba(100, 100, 200, 0.25)",
  black: "#383a42",
  red: "#cc0000",
  green: "#006600",
  yellow: "#8a6000",
  ...
  brightYellow: "#aa7a00",
  brightBlue: "#2277dd",
  brightMagenta: "#aa22aa",
  brightCyan: "#0088aa",
  brightWhite: "#24292e"
};
function getObsidianTheme() {
  const hasDocument = typeof document !== "undefined" && document.body;
  const isDark = hasDocument ? document.body.classList.contains("theme-dark") : false;
  const fallback = isDark ? FALLBACK_DARK_THEME : FALLBACK_LIGHT_THEME;
  if (!hasDocument) return { ...fallback };
  const style = getComputedStyle(document.body);
  const bg = style.getPropertyValue("--background-primary").trim() || fallback.background;
  const fg = style.getPropertyValue("--text-normal").trim() || fallback.foreground;
  const faint = style.getPropertyValue("--text-faint").trim() || fallback.brightBlack;
  const accent = style.getPropertyValue("--interactive-accent").trim() || fallback.cursor;
  if (isDark) {
    return {
      background: bg,
      foreground: fg,
...
var TermWrap = class {
  constructor(elem, opts = {}) {
    ...
    /** Set by TerminalView when the search overlay is visible. */
    this._searchBarVisible = false;
    this.opts = opts;
    const theme = opts.theme ?? getObsidianTheme();
    ...
    this.searchAddon = new import_addon_search.SearchAddon();
    this.serializeAddon = new import_addon_serialize.SerializeAddon();
    const webLinksAddon = new import_addon_web_links.WebLinksAddon();
    ...
    this.terminal.attachCustomKeyEventHandler((ev) => {
      if (ev.type === "keydown" && (ev.metaKey || ev.ctrlKey && ev.shiftKey) && String(ev.key || "").toLowerCase() === "c" && this.terminal.hasSelection?.()) {
        this.copyTerminalSelection();
        return false;
      }
      if ((ev.metaKey || ev.ctrlKey) && ev.key === "f") return false;
      if (ev.key === "Escape" && this._searchBarVisible) return false;
      return true;
    });
  ...
  // ── Process wiring ──────────────────────────────────────────────────────────
  /**
   * Wire a spawned Node.js child process (the Python PTY helper) to this terminal.
   *
   * - stdout → terminal.write()
   * - terminal.onData() → proc.stdin.write()
   * - terminal.onResize() → OSC escape to stdin: `\x1b]R;<cols>;<rows>\x07`
   * - proc 'exit' → opts.onClose?.()
   *
   * Disposables are tracked and cleaned up in dispose().
   */
  connectToProcess(proc) {
    if (!proc.stdout || !proc.stdin) {
      console.error("[TermWrap] connectToProcess: child process missing stdout or stdin");
      return;
    }
    proc.stdout.on("data", (data) => {
      if (!this.disposed) {
        this.terminal.write(data instanceof Buffer ? data : String(data));
      }
    });
    const inputDisposable = this.terminal.onData((data) => {
      if (!this.disposed && proc.stdin && !proc.stdin.destroyed) {
        proc.stdin.write(data);
      }
    });
    this.processDisposables.push(inputDisposable);
    const resizeDisposable = this.terminal.onResize(({ cols, rows }) => {
      if (!this.disposed && proc.stdin && !proc.stdin.destroyed) {
        proc.stdin.write("\x1B]R;" + cols + ";" + rows + "\x07");
      }
    });
    this.processDisposables.push(resizeDisposable);
    proc.on("exit", () => {
      if (!this.disposed) {
        this.opts.onClose?.();
      }
  ...
  // ── Theme ───────────────────────────────────────────────────────────────────
  /**
   * Re-read the current Obsidian CSS variables and apply the updated palette
   * to the terminal. Call this when Obsidian switches between dark/light mode.
   */
  updateTheme() {
  ...
  /**
   * Expose the SearchAddon's result-change event so TerminalView can update
   * the "N of M" match counter in the search overlay.
   */
  get onDidChangeResults() {
    return this.searchAddon.onDidChangeResults;
  }
  // ── Serialize / Restore ───────────────────────────────────────────────────
  serialize() {
    if (this.disposed) return "";
  ...
  dispose() {
    ...
    for (const d of this.processDisposables) {
      try {
        d.dispose();
      } catch {
      }
    }
    this.processDisposables.length = 0;
    this.terminal.dispose();
  }
};

// src/ptyHelper.ts
var PTY_HELPER_SSH = `#!/usr/bin/env python3
"""
Workbench PTY SSH helper \u2014 spawns an SSH session inside a proper PTY.

Environment variables:
  GSD_SSH_HOST     (required) SSH host to connect to
  GSD_TMUX_SESSION (optional) tmux session to attach; if set, attaches instead
                              of dropping to a plain shell

Resize protocol (sent by xterm.js side over stdin):
  ESC ] R ; <cols> ; <rows> BEL
  i.e. bytes: \\x1b]R;<cols>;<rows>\\x07

Exit: exits with the child's exit code.
"""
...
SSH_HOST = os.environ.get("GSD_SSH_HOST", "")
if not SSH_HOST:
    sys.stderr.write("Workbench PTY helper: GSD_SSH_HOST not set\\n")
    sys.exit(1)

TMUX_SESSION = os.environ.get("GSD_TMUX_SESSION", "")
SSH_KEY_PATH = os.environ.get("GSD_SSH_KEY_PATH", "")
...
SSH_CMD = [
    ...
    "-o", "ControlPath=/tmp/gsd-mux-%r@%h:%p",
...
if SSH_KEY_PATH:
    SSH_CMD[1:1] = ["-i", SSH_KEY_PATH]

if TMUX_SESSION:
    SSH_CMD += ["TERM=xterm-256color", "tmux", "attach-session", "-t", TMUX_SESSION]

# ---------------------------------------------------------------------------
# TIOCSWINSZ helper \u2014 resize the PTY
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Resize escape sequence parser state
# ---------------------------------------------------------------------------

RESIZE_PREFIX = b"\\x1b]R;"   # Note: this is the literal bytes ESC ] R ;
_resize_buf = b""

def _parse_resize(data, master_fd, child_pid):
    """
    Scan data for \\x1b]R;<cols>;<rows>\\x07 sequences.
    Returns the data with resize sequences stripped out.
    """
    global _resize_buf
    ...
    while i < len(data):
        if _resize_buf:
            # We are mid-sequence: accumulate until BEL or clearly broken
            _resize_buf += data[i:i+1]
            i += 1
            if BEL in _resize_buf:
                seq = _resize_buf[:_resize_buf.index(BEL)]
                _resize_buf = b""
                # seq should be \\x1b]R;<cols>;<rows>
                try:
                    parts = seq.split(b";")
                    # parts[0] = b"\\x1b]R", parts[1] = cols, parts[2] = rows
                    if len(parts) == 3:
                        cols = int(parts[1])
                        rows = int(parts[2])
                    ...
                    pass
            elif len(_resize_buf) > 64:
                # Bail out \u2014 not a real sequence
                out += _resize_buf
                _resize_buf = b""
        elif data[i:i+1] == ESC and data[i:i+4] == b"\\x1b]R;":
            # Start accumulating
            _resize_buf = data[i:i+4]
            i += 4
        else:
            out += data[i:i+1]
            i += 1
...
# ---------------------------------------------------------------------------
# Main: open PTY, fork, run select loop
# ---------------------------------------------------------------------------
...
def main():
    ...
    # Set stdin to raw/non-blocking pass-through
    try:
        ...
        termios.tcsetattr(sys.stdin.fileno(), termios.TCSAFLUSH, new_attrs)
        restore_tty = True
    except Exception:
        old_attrs = None
        restore_tty = False
    ...
    try:
        while True:
            ...
            if stdin_fd in r:
                ...
                if not data:
                    break
                # Strip and handle resize sequences before forwarding
                data = _parse_resize(data, master_fd, child_pid)
                if data:
                    try:
            ...
            if result[0] != 0:
                ...
                try:
                    ...
                    pass
                exit_code = (result[1] >> 8) & 0xFF
                sys.exit(exit_code)
    finally:
        if restore_tty and old_attrs is not None:
            try:
...
if __name__ == "__main__":
    main()
`;
function writePtyHelper() {
  const fs = require("fs");
  const path = require("path");
  const pid = process.pid;
  const tmpPath = path.join("/tmp", `gsd-pty-ssh-${pid}.py`);
  const scriptContent = PTY_HELPER_SSH.replace(/\\\\x1b/g, "\\x1b").replace(/\\\\x07/g, "\\x07");
  fs.writeFileSync(tmpPath, scriptContent, { mode: 493 });
  console.log("[Workbench] Wrote PTY helper to", tmpPath);
  return tmpPath;
}
function cleanupPtyHelper(filePath) {
  const fs = require("fs");
  try {
    fs.unlinkSync(filePath);
    console.log("[Workbench] Removed PTY helper:", filePath);
  } catch {
  }
}

// src/TerminalView.ts
var TERMINAL_VIEW_TYPE = "gsd-terminal-view";
var terminalStateMap = /* @__PURE__ */ new Map();
var TerminalView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    ...
    /** Debounce timer for tmux refresh-client after resize. */
    this.resizeRefreshTimer = null;
    /** Path to temp SSH key file written for this session (cleaned up on close). */
    this.tempKeyPath = "";
    /** Resolved workspace config for the current terminal session. */
    this.wsConfig = null;
    // ── Search bar state ──────────────────────────────────────────────────────
  ...
  getDisplayText() {
    if (this.workspaceName && this.projectPath) {
      return `Terminal: ${this.workspaceName}/${this.projectPath}`;
    }
    if (this.workspaceName) {
      return `Terminal: ${this.workspaceName}`;
    }
  ...
  async setState(state, _result) {
    this.workspaceName = state.workspaceName ?? "";
  ...
  async onOpen() {
    ...
    container.empty();
    container.addClass("gsd-terminal-container");
    const settings = this.plugin.settings;
    const configuredWorkspaces = this.plugin.getConfiguredWorkspaces();
    const coderUser = this.plugin.getConfiguredCoderUser();
    const wsConfig = this.workspaceName ? configuredWorkspaces.find((w) => w.coderName === this.workspaceName) : configuredWorkspaces[0];
    this.wsConfig = wsConfig ?? null;
    if (!wsConfig || !coderUser) {
      container.createDiv({
        cls: "gsd-terminal-banner",
        text: "No workspace configured. Open Settings \u2192 Can Workbench to add a workspace and set your Coder username."
      });
      return;
    }
    if (!this.workspaceName) {
      this.workspaceName = wsConfig.coderName;
    }
    if (!this.projectPath && wsConfig.projects.length > 0) {
      this.projectPath = wsConfig.projects[0].path;
    }
    this.sshHost = workspaceSshHost(wsConfig.coderName, coderUser);
    this.tmuxSession = String(this.projectPath || wsConfig.coderName || "main")
      .split(/[\\/]/)
      .filter(Boolean)
      .pop()
      .toLowerCase()
      ...
      .replace(/-+/g, "-")
      .replace(/^-+|-+$/g, "") || "main";
    this.searchBarEl = container.createDiv({ cls: "gsd-search-bar" });
    this.searchBarEl.style.display = "none";
    this.searchInputEl = this.searchBarEl.createEl("input", {
      cls: "gsd-search-input",
      attr: { type: "text", placeholder: "Search\u2026", spellcheck: "false" }
    });
    this.searchCountEl = this.searchBarEl.createSpan({ cls: "gsd-search-count", text: "" });
    const prevBtn = this.searchBarEl.createEl("button", { cls: "gsd-search-btn", attr: { "aria-label": "Previous match" } });
    prevBtn.textContent = "\u25B2";
    const nextBtn = this.searchBarEl.createEl("button", { cls: "gsd-search-btn", attr: { "aria-label": "Next match" } });
    nextBtn.textContent = "\u25BC";
    const closeBtn = this.searchBarEl.createEl("button", { cls: "gsd-search-btn gsd-search-close", attr: { "aria-label": "Close search" } });
    closeBtn.textContent = "\u2715";
    prevBtn.addEventListener("click", () => this.searchPrev());
    nextBtn.addEventListener("click", () => this.searchNext());
    closeBtn.addEventListener("click", () => this.toggleSearchBar(false));
    ...
    this.searchInputEl.addEventListener("input", () => {
      ...
      if (this.termWrap) {
        if (query) {
          this.termWrap.findNext(query, { incremental: true, decorations: { matchOverviewRuler: "#f97316", activeMatchColorOverviewRuler: "#f97316" } });
        } else {
          this.termWrap.clearSearch();
    ...
    const hostEl = container.createDiv({ cls: "gsd-terminal-host" });
    await this.connect(hostEl);
  }
  async onClose() {
    this.intentionalClose = true;
    if (this.searchResultsDisposable) {
      this.searchResultsDisposable.dispose();
      this.searchResultsDisposable = null;
    }
    if (this.termWrap) {
      try {
        const stateKey = `${this.workspaceName}/${this.projectPath}`;
        const serialized = this.termWrap.serialize();
        if (serialized) {
          terminalStateMap.set(stateKey, serialized);
        }
      } catch {
      }
      this.termWrap.dispose();
      this.termWrap = null;
    }
    this.killProcess();
    if (this.tempKeyPath) {
  ...
  // ── Search overlay ─────────────────────────────────────────────────────────
  /**
   * Show or hide the search overlay bar.  When shown, the text input is
   * focused and any existing query is selected.  When hidden, search
   * decorations are cleared and keyboard focus returns to the terminal.
   */
  toggleSearchBar(show) {
    if (!this.searchBarEl || !this.searchInputEl) return;
    this.searchBarVisible = show;
    if (this.termWrap) {
      this.termWrap._searchBarVisible = show;
    }
    if (show) {
      this.searchBarEl.style.display = "flex";
      this.searchInputEl.focus();
      this.searchInputEl.select();
      if (!this.searchResultsDisposable && this.termWrap) {
        this.searchResultsDisposable = this.termWrap.onDidChangeResults(
          ({ resultIndex, resultCount }) => {
            if (!this.searchCountEl) return;
            if (resultCount === 0) {
              this.searchCountEl.textContent = this.searchInputEl?.value ? "0 of 0" : "";
            } else {
              this.searchCountEl.textContent = `${resultIndex + 1} of ${resultCount}`;
            }
          }
        );
      }
      ...
      this.termWrap?.terminal.focus();
    }
  }
  searchNext() {
    const query = this.searchInputEl?.value ?? "";
    if (query && this.termWrap) {
      this.termWrap.findNext(query, { decorations: { matchOverviewRuler: "#f97316", activeMatchColorOverviewRuler: "#f97316" } });
    }
  }
  searchPrev() {
    const query = this.searchInputEl?.value ?? "";
    if (query && this.termWrap) {
      this.termWrap.findPrevious(query, { decorations: { matchOverviewRuler: "#f97316", activeMatchColorOverviewRuler: "#f97316" } });
    }
  }
  // ── Tab activation ─────────────────────────────────────────────────────────
  /**
   * Called by the plugin when this terminal tab becomes the active leaf.
   * Forces a re-fit (since hidden tabs may have had zero dimensions) and
   * returns keyboard focus to the terminal.
   */
  onBecameActive() {
    if (this.termWrap) {
      this.termWrap.fit(true);
      this.termWrap.terminal.focus();
    }
  }
  // ── Connection flow ───────────────────────────────────────────────────────
  async connect(hostEl) {
    this.termWrap = new TermWrap(hostEl, {
      theme: getObsidianTheme(),
      onResize: (cols, rows) => {
        this.onTermResize(cols, rows);
      },
      onClose: () => {
        this.onProcessExit();
      }
    ...
    this.termWrap.write(
      `\x1B[38;5;242mWorkspace: ${this.workspaceName}\r
Project:   ${this.projectPath || "(default)"}\r
Connecting...\x1B[0m\r
`
    );
    const stateKey = `${this.workspaceName}/${this.projectPath}`;
    if (terminalStateMap.has(stateKey)) {
      this.termWrap.restoreState(terminalStateMap.get(stateKey));
      terminalStateMap.delete(stateKey);
    }
    try {
      const tmuxName = sanitizeShellArg(this.tmuxSession);
      const result = await execInWorkspace(
        this.sshHost,
        `tmux has-session -t ${tmuxName} 2>/dev/null && echo exists || echo missing`
      );
      if (result.trim() === "missing") {
        this.termWrap.write(`\x1B[38;5;242mCreating session: ${tmuxName}\x1B[0m\r
`);
        const safePath = sanitizeShellArg(this.projectPath);
        const cdTarget = safePath ? `~/${safePath}` : "~";
        await execInWorkspace(
          this.sshHost,
          `tmux new-session -d -s ${tmuxName} -c ${cdTarget}`
        );
      } else {
        this.termWrap.write(`\x1B[38;5;242mReattaching to session: ${tmuxName}\x1B[0m\r
`);
      }
      await execInWorkspace(
        this.sshHost,
        `tmux set-window-option -t ${tmuxName} window-size latest 2>/dev/null; tmux set-option -t ${tmuxName} mouse on 2>/dev/null; tmux set-option -t ${tmuxName} aggressive-resize off 2>/dev/null; true`
      );
      this.spawnPtyHelper();
    } catch (err) {
      const msg = err?.message || String(err);
      this.termWrap.write(`\r
\x1B[31mConnection failed: ${msg}\x1B[0m\r
`);
      this.termWrap.write(`\x1B[38;5;242m[Press any key to retry]\x1B[0m`);
      this.connected = false;
      const disposable = this.termWrap.handleInput(() => {
        disposable.dispose();
        this.reconnect();
      });
    }
  ...
  spawnPtyHelper() {
    ...
    const fs = require("fs");
    if (!this.plugin.ptyHelperPath) {
      this.plugin.ptyHelperPath = writePtyHelper();
    }
    const dims = this.termWrap.fitAddon.proposeDimensions();
    ...
    const spawnEnv = {
      ...
      PATH: shellPath(),
      TERM: "xterm-256color",
      GSD_SSH_HOST: this.sshHost,
      GSD_TMUX_SESSION: this.tmuxSession,
      GSD_TERM_COLS: String(cols),
      GSD_TERM_ROWS: String(rows)
    };
    if (this.wsConfig?.hasKey) {
      try {
        const pluginDir = this.plugin.getPluginDir();
        const keyData = getSecret(pluginDir, SECRET_KEYS.sshKey(this.wsConfig.coderName));
        if (keyData) {
          this.tempKeyPath = `/tmp/gsd-ssh-key-${this.wsConfig.coderName}`;
          fs.writeFileSync(this.tempKeyPath, keyData + "\n", { mode: 384 });
          spawnEnv["GSD_SSH_KEY_PATH"] = this.tempKeyPath;
        }
      } catch (err) {
        console.error("[Workbench] Failed to write temp SSH key:", err);
      }
    }
    this.proc = spawn("python3", [this.plugin.ptyHelperPath], {
      env: spawnEnv,
      stdio: ["pipe", "pipe", "pipe"]
    });
    this.termWrap.connectToProcess(this.proc);
    this.connected = true;
    if (this.proc.stderr) {
      this.proc.stderr.on("data", (data) => {
        console.error("[Workbench PTY stderr]", data.toString());
      });
    }
    this.termWrap.fit(true);
    setTimeout(() => this.termWrap?.fit(true), 100);
    setTimeout(() => this.termWrap?.fit(true), 300);
    setTimeout(() => {
      if (this.termWrap) {
        this.termWrap.fit(true);
      }
    }, 500);
    setTimeout(() => this.termWrap?.fit(true), 1e3);
  }
  // ── Resize propagation ────────────────────────────────────────────────────
  onTermResize(cols, rows) {
    if (!this.connected || !this.sshHost || !this.tmuxSession) return;
    if (this.resizeRefreshTimer) {
      clearTimeout(this.resizeRefreshTimer);
    }
    const tmuxName = sanitizeShellArg(this.tmuxSession);
    this.resizeRefreshTimer = setTimeout(() => {
      execInWorkspace(
        this.sshHost,
        `tmux refresh-client -t ${tmuxName} 2>/dev/null; true`
      ).catch(() => {
      });
    }, 200);
  }
...
\x1B[38;5;242m[Connection closed \u2014 press any key to reconnect]\x1B[0m`
  ...
  reconnect() {
    ...
    if (!this.termWrap) return;
    this.termWrap.write(`\r
\x1B[38;5;242mReconnecting...\x1B[0m\r
`);
    this.intentionalClose = false;
    const tmuxName = sanitizeShellArg(this.tmuxSession);
    execInWorkspace(
      ...
      if (this.termWrap) {
        this.termWrap.write(`\r
\x1B[31mReconnect failed: ${msg}\x1B[0m\r
`);
        this.termWrap.write(`\x1B[38;5;242m[Press any key to retry]\x1B[0m`);
        const disposable = this.termWrap.handleInput(() => {
          disposable.dispose();
...
for event_log in glob.glob(home + "/**/.gsd/event-log.jsonl", recursive=True):
    ...
    # Read STATE.md
    try:
        data["state"] = open(os.path.join(gsd_dir, "STATE.md")).read()
    except:
        data["state"] = ""

    # Tmux sessions with attached flag
    try:
        tmux = subprocess.run(
        ...
        for line in (tmux.stdout.strip().split("\\n") if tmux.stdout.strip() else []):
            ...
            attached_count = int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else 0
            attached = attached_count > 0
            idle_secs = now - activity
            all_sessions.append({"name": sname, "idle": idle_secs, "attached": attached, "attachedCount": attached_count})
        # Filter to sessions matching this project
        pslug = data["path"].replace("/", "-")
        project_sessions = [
            s for s in all_sessions
            if s["name"] == project_name
            or s["name"] == pslug
            or s["name"].startswith("gsd-term-" + project_name)
            or project_name in s["name"]
        ]
        project_sessions = [s for s in project_sessions if s["name"] != "gsd-watcher"]
        data["tmuxSessions"] = project_sessions

        # Capture last 5 lines from the most active session
        if project_sessions:
    ...
    # Last event from event-log.jsonl
    try:
        lines = open(event_log).readlines()
        if lines:
            data["lastEvent"] = json.loads(lines[-1])
        else:
            data["lastEvent"] = None
    except:
        data["lastEvent"] = None
...
`.trim();
function parseStateMd(state) {
  const result = {};
  const milestoneMatch = state.match(/\*\*Active Milestone:\*\*\s*(.+)/);
  if (milestoneMatch && milestoneMatch[1].trim() !== "None") {
    result.milestone = milestoneMatch[1].trim();
  }
  const sliceMatch = state.match(/\*\*Active Slice:\*\*\s*(.+)/);
  ...
  // Parse sections to count slices and milestones separately.
  // STATE.md typically has sections like "## Slices" and "## Milestones".
  function countInSection(headerRegex) {
    const match = state.match(headerRegex);
    if (!match) return null;
    // Capture from after the header to the next section or end
    const startIdx = match.index + match[0].length;
    const rest = state.substring(startIdx);
    const nextSection = rest.match(/\n##\s/);
    const body = nextSection ? rest.substring(0, nextSection.index) : rest;
    const completed = (body.match(/- ✅/g) || []).length;
    const inProgress = (body.match(/- 🔄/g) || []).length;
  ...
  const sliceSection = countInSection(/##\s+Slices?\b/i);
  const milestoneSection = countInSection(/##\s+Milestones?\b/i);

  if (sliceSection) {
    result.sliceCurrent = sliceSection.completed;
    result.sliceTotal = sliceSection.total;
    result.progressType = "slices";
  } else if (milestoneSection) {
    // No slice section found — fall back to milestone progress
    result.sliceCurrent = milestoneSection.completed;
    result.sliceTotal = milestoneSection.total;
    result.progressType = "milestones";
  } else {
    // Last resort: count ALL checkmarks and label generically
    ...
    if (total > 0) {
      ...
      result.progressType = "items";
    }
  }
  // Look for archived/completed milestones in the state file
  const archivedMatch = state.match(/(?:Completed|Archived|Finished)\s+Milestones?:?\s*\n?([\s\S]*?)(?=\n#|\n\*\*|$)/i);
  if (archivedMatch) {
    const items = archivedMatch[1].match(/[-*]\s+(.+)/g);
    if (items && items.length > 0) {
      result.completedMilestones = items.length;
      result.lastCompletedMilestone = items[items.length - 1].replace(/^[-*]\s+/, "").trim();
    }
  }
  // If phase is "complete" and there's no active milestone, infer milestone is done
  if (!result.milestone && phaseMatch && /complete/i.test(phaseMatch[1])) {
    result.milestoneStatus = "complete";
  } else if (!result.milestone) {
    result.milestoneStatus = "none";
  } else {
    result.milestoneStatus = "active";
  }
  return result;
}
async function pollWorkspaces(settings) {
  const allStatuses = [];
  const workspaces = getConfiguredStatusWorkspaces(settings);
  const coderUser = getConfiguredStatusCoderUser(settings);
  if (!coderUser) {
    for (const ws of workspaces) allStatuses.push(...buildConfiguredStatusFallback(ws, "Coder username not configured"));
    return allStatuses;
  }
  const results = await Promise.allSettled(
    workspaces.map(async (ws) => {
      try {
        const host = workspaceSshHost(ws.coderName, coderUser);
        const escapedScript = GSD_FETCH_SCRIPT_LITE.replace(/'/g, `'"'"'`);
        const command = `python3 -c '${escapedScript}'`;
        const raw = await execInWorkspace(host, command);
        if (!raw || !raw.trim()) return buildConfiguredStatusFallback(ws);
        let projects;
        try {
          projects = JSON.parse(raw.trim());
        } catch {
          console.warn(
            `[statusPoller] Failed to parse JSON from ${ws.coderName}:`,
            raw.substring(0, 200)
          );
          return buildConfiguredStatusFallback(ws, "Could not parse workspace status");
        }
        const statuses = [];
        for (const proj of (ws.projects || []).filter((p) => p && p.type !== "separator")) {
          ...
          if (remote) {
            const stateInfo = parseStateMd(remote.state || "");
            const sessions = (remote.tmuxSessions || []).map(
              (s) => ({
                name: s.name,
                idle: s.idle,
                attached: s.attached
            ...
            const autoModeSession = sessions.find(
              (s) => s.name === pslug || s.name === proj.path.split("/").pop()
            );
            const autoMode = !!autoModeSession && autoModeSession.idle < 120;
            const isRunning = sessions.some((s) => s.idle < 120);
            statuses.push({
              id: sessionId,
              workspace: ws.coderName,
              project: proj.path,
              displayName: proj.displayName,
              milestone: stateInfo.milestone ?? null,
              milestoneStatus: stateInfo.milestoneStatus ?? "none",
              slice: stateInfo.slice ?? null,
              phase: stateInfo.phase ?? null,
              sliceCurrent: stateInfo.sliceCurrent ?? 0,
              sliceTotal: stateInfo.sliceTotal ?? 0,
              progressType: stateInfo.progressType ?? "items",
              autoMode,
              isRunning,
              tmuxSessions: sessions,
              terminalPreview: remote.terminalPreview || [],
              gitBranch: remote.gitBranch || null,
              lastAction: remote.lastEvent || null
            });
          } else {
            statuses.push(...buildConfiguredStatusFallback({ ...ws, projects: [proj] }));
          }
        }
        return statuses;
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[statusPoller] Workspace ${ws.coderName} unavailable:`, message);
        return buildConfiguredStatusFallback(ws, message);
      }
    })
  );
...
function getConfiguredStatusWorkspaces(settings) {
  const topLevel = settings?.workspaces;
  const nested = settings?.gsd?.workspaces;
  if (Array.isArray(topLevel) && topLevel.length > 0) return getDefaultVisibleWorkspaces(topLevel);
  if (Array.isArray(nested) && nested.length > 0) return getDefaultVisibleWorkspaces(nested);
  if (Array.isArray(topLevel)) return getDefaultVisibleWorkspaces(topLevel);
  if (Array.isArray(nested)) return getDefaultVisibleWorkspaces(nested);
  return [];
}
...
function getDefaultVisibleWorkspaces(workspaces) {
  return (workspaces || []).filter(isDefaultVisibleWorkspace);
}
function getConfiguredStatusCoderUser(settings) {
  return settings?.coderUser || settings?.gsd?.coderUser || "";
}
function buildConfiguredStatusFallback(ws, connectionError = "") {
  return (ws?.projects || [])
    .filter((proj) => proj && proj.type !== "separator")
    .map((proj) => ({
      id: `${ws.coderName}:${proj.path || ""}`,
      workspace: ws.coderName,
      project: proj.path || "",
      displayName: proj.displayName || proj.path || ws.displayName || ws.coderName,
      milestone: null,
      milestoneStatus: "none",
      slice: null,
      phase: connectionError ? "unavailable" : null,
      sliceCurrent: 0,
      sliceTotal: 0,
      progressType: "items",
      autoMode: false,
      isRunning: false,
      tmuxSessions: [],
      terminalPreview: connectionError ? [`Workspace unavailable: ${connectionError}`] : [],
      gitBranch: null,
      lastAction: null
    }));
}
...
// src/StatusView.ts
var STATUS_VIEW_TYPE = "gsd-status-view";
var URGENCY_DOT = {
  error: "var(--color-red)",
...
function getCardUrgency(project) {
  const hasRecentActivity = project.tmuxSessions.some((s) => s.idle < 120);
  if (project.phase === "error" || project.phase === "blocked") {
    return hasRecentActivity ? "error" : "idle";
  }
  if (project.phase === "evaluating-gates" || project.phase === "waiting") {
    return hasRecentActivity ? "warning" : "idle";
  }
  if (project.isRunning && project.autoMode && hasRecentActivity) return "active";
  if (project.isRunning && hasRecentActivity) return "active";
  if (project.phase === "complete") return "complete";
  return "idle";
}
function humanizeIdle(seconds) {
...
var StatusView = class extends import_obsidian3.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.pollInterval = null;
    this.projects = [];
    this.selectedKey = null;
    /** navigation = false → opens as a tab, not in a sidebar */
  ...
  // ── Polling ─────────────────────────────────────────────────────────────
  /** Poll all workspaces, preserve / auto-select selectedKey, then re-render. */
  async doPoll() {
    try {
      this.projects = await pollWorkspaces(this.plugin.settings);
    } catch (err) {
      console.error("[Workspace StatusView] Poll failed:", err);
      this.projects = [];
    }
  ...
  // ── Rendering ────────────────────────────────────────────────────────────
  /**
   * Re-render the full two-panel layout inside root.
   * Called after every poll and after every selection change.
   */
  renderAll() {
    const root = this.containerEl.children[1];
    root.empty();
    root.addClass("gsd-status-root");
    // Render a top toolbar with refresh button (replaces the old sidebar)
    const toolbar = root.createDiv({ cls: "gsd-status-toolbar" });
    toolbar.createEl("h3", { text: "Projects Dashboard", cls: "gsd-status-toolbar-title" });
    const refreshBtn = toolbar.createEl("button", { cls: "gsd-status-btn", text: "\u21BB Refresh" });
    refreshBtn.addEventListener("click", () => this.doPoll());
    // Render all projects as a grid
    this.renderDetailPanel(root);
  }
  // ── Left Panel: Session List ─────────────────────────────────────────────
  renderSessionList(root) {
    const panel = root.createDiv({ cls: "gsd-status-session-list" });
    const header = panel.createDiv({ cls: "gsd-status-list-header" });
    header.createSpan({ cls: "gsd-status-list-title", text: "Projects" });
    const refreshBtn = header.createSpan({ cls: "gsd-status-list-refresh" });
    ...
    if (this.projects.length === 0) {
      panel.createDiv({
        cls: "gsd-status-list-empty",
        text: "No projects found."
      });
      return;
    }
    const workspaces = [...new Set(this.projects.map((p) => p.workspace))];
    const multiWorkspace = workspaces.length > 1;
    if (multiWorkspace) {
      for (const ws of workspaces) {
        const wsProjects = this.projects.filter((p) => p.workspace === ws);
        panel.createDiv({ cls: "gsd-status-list-workspace-label", text: ws });
        for (const project of wsProjects) {
          this.renderListRow(panel, project);
        }
      }
    } else {
  ...
  renderListRow(parent, project) {
    const urgency = getCardUrgency(project);
    const key = projectKey(project);
    const isActive = this.selectedKey === key;
    const row = parent.createDiv({
      cls: `gsd-status-list-row${isActive ? " is-active" : ""}`
    });
    const dot = row.createSpan({ cls: "gsd-status-list-dot" });
    dot.style.backgroundColor = URGENCY_DOT[urgency];
    row.createSpan({ cls: "gsd-status-list-name", text: project.displayName });
    if (project.autoMode) {
      row.createSpan({ cls: "gsd-status-list-auto" });
    }
  ...
  renderDetailPanel(root) {
    const panel = root.createDiv({ cls: "gsd-status-detail-panel" });
    if (!this.projects || this.projects.length === 0) {
      panel.createDiv({
        cls: "gsd-status-detail-empty",
        text: "No projects found."
      });
      return;
    }
    // Render ALL projects as a dashboard grid
    const grid = panel.createDiv({ cls: "gsd-status-dashboard-grid" });
    for (const project of this.projects) {
      this.renderProjectCard(grid, project);
    }
  }
  // ── Project Card (right-panel card content) ──────────────────────────────
  /** Render a single project status card into the given parent element. */
  renderProjectCard(parent, project) {
    const urgency = getCardUrgency(project);
    const card = parent.createDiv({ cls: `gsd-status-card gsd-urgency-${urgency}` });
    const header = card.createDiv({ cls: "gsd-status-header" });
    const headerDot = header.createSpan({ cls: "gsd-urgency-dot" });
    headerDot.style.backgroundColor = URGENCY_DOT[urgency];
    header.createSpan({ cls: "gsd-status-project-name", text: project.displayName });
    const statusRow = card.createDiv({ cls: "gsd-status-row" });
    statusRow.createSpan({ cls: "gsd-status-label", text: "Milestone " });
    if (project.milestone) {
      statusRow.createSpan({ text: project.milestone });
    } else if (project.milestoneStatus === "complete") {
      statusRow.createSpan({ cls: "gsd-status-complete", text: "\u2713 complete" });
    } else {
      statusRow.createSpan({ cls: "gsd-status-muted", text: "not set" });
    }
    if (project.slice) {
      statusRow.createSpan({ cls: "gsd-status-label", text: "  Slice " });
      statusRow.createSpan({ text: project.slice });
    }
    if (project.phase) {
      statusRow.createSpan({ cls: "gsd-status-label", text: "  Phase " });
      statusRow.createSpan({ text: project.phase });
    }
    if (project.sliceTotal > 0) {
      const progressWrap = card.createDiv({ cls: "gsd-status-progress" });
      const pct = Math.round(project.sliceCurrent / project.sliceTotal * 100);
      const fill = progressWrap.createDiv({ cls: "gsd-status-progress-fill" });
      fill.style.width = `${pct}%`;
      const label = project.progressType || "items";
      card.createDiv({
        cls: "gsd-status-row",
        text: `${project.sliceCurrent} / ${project.sliceTotal} ${label}`
      });
    }
    if (project.tmuxSessions.length > 0) {
      card.createDiv({ cls: "gsd-status-section-title", text: "Sessions" });
      const sessionsDiv = card.createDiv({ cls: "gsd-status-sessions" });
      for (const session of project.tmuxSessions) {
        const row = sessionsDiv.createDiv({ cls: "gsd-status-session-row" });
        row.createSpan({ text: session.name });
        row.createSpan({ cls: "gsd-status-badge", text: humanizeIdle(session.idle) });
        if (session.attached) {
          row.createSpan({ cls: "gsd-status-badge gsd-status-badge-attached", text: `clients ${session.attachedCount || 1}` });
        }
        const killBtn = row.createEl("button", {
          cls: "gsd-status-btn gsd-status-btn-danger",
          text: "Kill"
        });
        killBtn.addEventListener("click", () => {
          this.killSession(project, session);
        });
      }
    }
    if (project.gitBranch) {
      card.createSpan({ cls: "gsd-status-badge", text: `\u2387 ${project.gitBranch}` });
    }
  }
  // ── Action methods ──────────────────────────────────────────────────────
  /** Start auto-mode in a new tmux session for the given project. */
  async startAuto(project) {
    try {
      const settings = this.plugin.settings;
      const host = workspaceSshHost(project.workspace, getConfiguredStatusCoderUser(settings));
      const sessionName = sanitizeShellArg(project.project.replace(/\//g, "-"));
      await execInWorkspace(
        host,
      ...
      await execInWorkspace(
        host,
        `tmux new-session -d -s ${sessionName} 'cd ~/${project.project} && gsd auto'`
      );
      new import_obsidian3.Notice(`Started auto-mode for ${project.displayName}`);
      await this.doPoll();
    } catch (err) {
      console.error("[Workspace StatusView] startAuto failed:", err);
      new import_obsidian3.Notice(`Failed to start auto-mode for ${project.displayName}`);
    }
  }
  /** Stop auto-mode by sending Ctrl-C then killing the tmux session. */
  async stopAuto(project) {
    try {
      const settings = this.plugin.settings;
      const host = workspaceSshHost(project.workspace, getConfiguredStatusCoderUser(settings));
      const sessionName = sanitizeShellArg(project.project.replace(/\//g, "-"));
      await execInWorkspace(
        host,
        `tmux send-keys -t ${sessionName} C-c 2>/dev/null; sleep 2; tmux kill-session -t ${sessionName} 2>/dev/null; true`
      );
      new import_obsidian3.Notice(`Stopped auto-mode for ${project.displayName}`);
      await this.doPoll();
    } catch (err) {
      console.error("[Workspace StatusView] stopAuto failed:", err);
      new import_obsidian3.Notice(`Failed to stop auto-mode for ${project.displayName}`);
    }
  }
  /** Kill a specific tmux session. */
  async killSession(project, session) {
    try {
      const settings = this.plugin.settings;
      const host = workspaceSshHost(project.workspace, getConfiguredStatusCoderUser(settings));
      const safeName = sanitizeShellArg(session.name);
      await execInWorkspace(
        host,
        `tmux kill-session -t '${safeName}' 2>/dev/null; true`
      );
      new import_obsidian3.Notice(`Killed session '${session.name}'`);
      await this.doPoll();
    } catch (err) {
      console.error("[Workspace StatusView] killSession failed:", err);
      new import_obsidian3.Notice(`Failed to kill session '${session.name}'`);
    }
  }
  /**
   * Called by main.ts on Obsidian theme changes.
   * No-op — DOM uses CSS variables and auto-adapts.
   */
  updateTheme() {
  }
};
...
var EXPLORER_VIEW_TYPE = "gsd-explorer-view";
...
var MAX_PREVIEW_SIZE = 1048576;
var PREVIEW_BYTE_LIMIT = 5e4;
...
function fileIcon(name, isDir, isSymlink) {
  if (isDir) return "\u{1F4C1}";
  if (isSymlink) return "\u{1F517}";
...
function resolvePath(current, segment) {
  if (segment === "..") {
    const p = current.replace(/\/+$/, "").split("/");
    p.pop();
...
var ExplorerView = class extends import_obsidian4.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    /** Prevents Obsidian from injecting its own navigation arrows. */
    this.navigation = false;
    // ── Selection state — persisted via getState/setState ──────────────────
    ...
    /** Root container element (the Obsidian content pane). */
    this.container = null;
    this.plugin = plugin;
  }
  getViewType() {
  ...
  getDisplayText() {
    if (this.selectedWorkspace && this.selectedProject) {
      const ws = this.plugin.getConfiguredWorkspaces().find(
        (w) => w.coderName === this.selectedWorkspace
      );
      const proj = ws?.projects.find((p) => p.path === this.selectedProject);
      const wsLabel = ws?.displayName || this.selectedWorkspace;
      const projLabel = proj?.displayName || this.selectedProject;
      return `Explorer: ${wsLabel} \u203A ${projLabel}`;
    }
    if (this.selectedWorkspace) {
      const ws = this.plugin.getConfiguredWorkspaces().find(
        (w) => w.coderName === this.selectedWorkspace
      );
      return `Explorer: ${ws?.displayName || this.selectedWorkspace}`;
    }
  ...
  async setState(state, _result) {
    const s = state;
    if (s.selectedWorkspace) this.selectedWorkspace = s.selectedWorkspace;
    if (s.selectedProject) this.selectedProject = s.selectedProject;
    if (s.currentPath) this.currentPath = s.currentPath;
    if (this.mounted) {
      this.renderAll();
      if (this.selectedWorkspace && this.selectedProject) {
  ...
  async onOpen() {
    ...
    const root = this.containerEl.children[1];
    root.empty();
    root.addClass("gsd-explorer-root");
    this.container = root;
  ...
  /** Called by onThemeChange in main.ts — redraw to pick up CSS variable changes. */
  updateTheme() {
    this.renderAll();
  }
  // ── Project selection ───────────────────────────────────────────────────
  /** Select a workspace+project, reset path, and reload the right panel. */
  async selectProject(workspaceName, projectPath) {
    this.selectedWorkspace = workspaceName;
    this.selectedProject = projectPath;
    if (projectPath) {
      this.currentPath = projectPath.startsWith("/") ? projectPath : `/home/coder/${projectPath}`;
    } else {
      this.currentPath = "/home";
    }
    this.closePreview();
  ...
  async fetchListing() {
    if (!this.selectedWorkspace) {
      this.error = "No workspace configured. Open Settings \u2192 Can Workbench.";
    ...
    this.renderDetailPanel(this.getDetailPanel());
    try {
      const host = workspaceSshHost(
        this.selectedWorkspace,
        this.plugin.getConfiguredCoderUser()
      );
      const escapedPath = escapeShellSingleQuote(this.currentPath);
      const command = `ls -la --group-directories-first '${escapedPath}'`;
      const output = await execInWorkspace(host, command);
  ...
  async handleFilePreview(entry) {
    const fullPath = this.currentPath.endsWith("/") ? this.currentPath + entry.name : this.currentPath + "/" + entry.name;
    ...
    if (!isNaN(sizeBytes) && sizeBytes > MAX_PREVIEW_SIZE) {
      ...
      this.previewContent = null;
      this.previewLoading = false;
      this.previewError = `File too large to preview (${humanSize(entry.size)})`;
    ...
    try {
      const mimeOutput = await execInWorkspace(
        host,
    ...
    try {
      const content = await execInWorkspace(
        host,
        `head -c ${PREVIEW_BYTE_LIMIT} '${escapedPath}'`
      );
      if (!this.mounted) return;
      ...
      this.previewContent = null;
      this.previewLoading = false;
      this.previewError = `Failed to read file: ${msg}`;
      this.previewIsText = true;
      this.previewMimeType = detectedMime;
      this.renderDetailPanel(this.getDetailPanel());
    }
  ...
  // ── Rendering ───────────────────────────────────────────────────────────
  /**
   * Full re-render: left session list + right detail panel.
   * Called on open, theme change, and project selection.
   */
  renderAll() {
    if (!this.container) return;
    this.container.empty();
    const sessionList = this.container.createDiv({
      cls: "gsd-explorer-session-list"
    });
    this.renderSessionList(sessionList);
    const detail = this.container.createDiv({
      cls: "gsd-explorer-detail-panel"
    });
    this.renderDetailPanel(detail);
  }
  /**
   * Return the existing detail panel element (created in renderAll).
   * Used by async callbacks to re-render only the right side.
   */
  getDetailPanel() {
    if (!this.container) {
      this.renderAll();
      return this.container.querySelector(
        ".gsd-explorer-detail-panel"
      );
    }
    let panel = this.container.querySelector(
      ".gsd-explorer-detail-panel"
    );
    if (!panel) {
      this.renderAll();
  ...
  // ── Left panel: session list ────────────────────────────────────────────
  renderSessionList(panel) {
    panel.empty();
    const workspaces = this.plugin.getConfiguredWorkspaces();
    const multiWorkspace = workspaces.length > 1;
    if (workspaces.length === 0) {
      const empty = panel.createDiv({ cls: "gsd-explorer-list-empty" });
      empty.createSpan({ text: "No workspaces configured." });
      return;
    }
    for (const ws of workspaces) {
      if (multiWorkspace) {
        const header = panel.createDiv({ cls: "gsd-explorer-list-header" });
        header.createSpan({
          text: ws.displayName || ws.coderName,
          cls: "gsd-explorer-list-header-label"
        });
      }
      if (ws.projects.length === 0) {
        this.renderSessionRow(panel, ws.coderName, "", ws.displayName || ws.coderName);
      } else {
  ...
  renderSessionRow(parent, workspaceName, projectPath, label) {
    const isActive = this.selectedWorkspace === workspaceName && this.selectedProject === projectPath;
    const row = parent.createDiv({
      cls: "gsd-explorer-list-row" + (isActive ? " is-active" : "")
    });
    row.createSpan({
      cls: "gsd-explorer-list-icon",
      text: "\u{1F4C1}"
    });
    row.createSpan({
      cls: "gsd-explorer-list-name",
      text: label
    });
    row.addEventListener("click", () => {
      this.selectProject(workspaceName, projectPath);
    });
  }
  // ── Right panel: detail ─────────────────────────────────────────────────
  renderDetailPanel(panel) {
    panel.empty();
    if (!this.selectedWorkspace || !this.selectedProject) {
      const empty = panel.createDiv({ cls: "gsd-explorer-detail-empty" });
      empty.createSpan({ text: "\u{1F4C2}", cls: "gsd-explorer-state-icon" });
      empty.createEl("p", { text: "Select a project from the left panel." });
      return;
    }
    const toolbar = panel.createDiv({ cls: "gsd-explorer-toolbar" });
    this.renderToolbar(toolbar);
    const content = panel.createDiv({ cls: "gsd-explorer-content" });
    if (this.error && !this.loading) {
      this.renderError(content);
    } else if (!this.error && !this.loading && this.entries.length === 0) {
  ...
  renderToolbar(toolbar) {
    ...
    const upBtn = toolbar.createEl("button", {
      cls: "gsd-explorer-btn",
      text: "\u2191",
      title: "Parent directory"
    });
    if (isRoot || this.loading) {
      upBtn.disabled = true;
      upBtn.addClass("gsd-explorer-btn-disabled");
    }
    upBtn.addEventListener("click", () => {
      this.navigateTo(resolvePath(this.currentPath, ".."));
    });
    ...
    if (this.loading) {
      ...
      refreshBtn.addClass("gsd-explorer-btn-disabled");
    }
    refreshBtn.addEventListener("click", () => {
      this.fetchListing();
    });
    const termBtn = toolbar.createEl("button", {
      cls: "gsd-explorer-btn",
      text: ">_",
      title: "Open terminal here"
    });
    termBtn.addEventListener("click", () => {
      this.plugin.openTerminalView(this.selectedWorkspace, this.currentPath);
    });
    const crumbs = toolbar.createDiv({ cls: "gsd-explorer-breadcrumbs" });
    const segments = pathSegments(this.currentPath);
    segments.forEach((seg, i) => {
      if (i > 0) {
        crumbs.createSpan({
          cls: "gsd-explorer-crumb-sep",
          text: "/"
        });
      }
      const btn = crumbs.createEl("button", {
        cls: "gsd-explorer-crumb" + (i === segments.length - 1 ? " gsd-explorer-crumb-active" : ""),
        text: seg.label
      });
      if (this.loading) btn.disabled = true;
      btn.addEventListener("click", () => {
        this.navigateTo(seg.path);
      });
    });
    if (this.loading) {
      toolbar.createDiv({ cls: "gsd-explorer-spinner" });
    }
  }
  renderError(parent) {
    const wrap = parent.createDiv({ cls: "gsd-explorer-state" });
    wrap.createEl("span", { text: "\u26A0\uFE0F", cls: "gsd-explorer-state-icon" });
    wrap.createEl("p", {
      text: "Failed to list directory",
      cls: "gsd-explorer-state-title gsd-explorer-error-text"
    });
    wrap.createEl("p", {
      text: this.error ?? "",
      cls: "gsd-explorer-state-detail"
    });
  ...
  renderEmpty(parent) {
    const wrap = parent.createDiv({ cls: "gsd-explorer-state" });
    wrap.createEl("span", { text: "\u{1F4C2}", cls: "gsd-explorer-state-icon" });
    wrap.createEl("p", {
      text: "Empty directory",
      cls: "gsd-explorer-state-title"
    });
  }
  renderLoading(parent) {
    const wrap = parent.createDiv({ cls: "gsd-explorer-state" });
    wrap.createDiv({ cls: "gsd-explorer-spinner gsd-explorer-spinner-lg" });
  }
  renderTable(parent) {
    const table = parent.createEl("table", { cls: "gsd-explorer-table" });
    const thead = table.createEl("thead");
    const headerRow = thead.createEl("tr");
    headerRow.createEl("th", { text: "Name", cls: "gsd-explorer-th-name" });
    headerRow.createEl("th", { text: "Size", cls: "gsd-explorer-th-size" });
    headerRow.createEl("th", {
      text: "Modified",
      cls: "gsd-explorer-th-date"
    });
    const tbody = table.createEl("tbody");
    for (const entry of this.entries) {
      const row = tbody.createEl("tr", { cls: "gsd-explorer-row" });
      const nameCell = row.createEl("td", { cls: "gsd-explorer-cell-name" });
      nameCell.createSpan({
        text: fileIcon(entry.name, entry.isDir, entry.isSymlink),
        cls: "gsd-explorer-icon"
      });
      nameCell.createSpan({
        text: entry.name,
        cls: entry.isDir ? "gsd-explorer-name-dir" : "gsd-explorer-name-file"
      });
      if (entry.isSymlink) {
        nameCell.createSpan({
          text: "\u2192",
          cls: "gsd-explorer-symlink-arrow"
        });
      }
      row.createEl("td", {
        text: entry.isDir ? "\u2014" : humanSize(entry.size),
        cls: "gsd-explorer-cell-size"
      });
      row.createEl("td", {
        text: entry.date,
        cls: "gsd-explorer-cell-date"
      });
      row.addEventListener("click", () => {
        if (entry.isDir || entry.isSymlink) {
          this.navigateTo(resolvePath(this.currentPath, entry.name));
        } else {
          this.handleFilePreview(entry);
        }
      });
    }
  }
  renderPreview(parent) {
    const panel = parent.createDiv({ cls: "gsd-explorer-preview" });
    const header = panel.createDiv({ cls: "gsd-explorer-preview-header" });
    const info = header.createDiv({ cls: "gsd-explorer-preview-info" });
    info.createSpan({
      text: this.previewFileName ?? "",
      cls: "gsd-explorer-preview-name"
    });
    info.createSpan({
      text: humanSize(this.previewFileSize),
      cls: "gsd-explorer-preview-size"
    });
    if (this.previewMimeType) {
      info.createSpan({
        text: this.previewMimeType,
        cls: "gsd-explorer-preview-badge"
      });
    }
    ...
    const content = panel.createDiv({ cls: "gsd-explorer-preview-content" });
    if (this.previewLoading) {
      const loadWrap = content.createDiv({
        cls: "gsd-explorer-preview-center"
      });
      loadWrap.createDiv({ cls: "gsd-explorer-spinner" });
      loadWrap.createSpan({
        text: "Loading preview\u2026",
        cls: "gsd-explorer-preview-msg"
      });
    } else if (this.previewError) {
      const errWrap = content.createDiv({
        cls: "gsd-explorer-preview-center"
      });
      errWrap.createSpan({
        text: this.previewError,
        cls: "gsd-explorer-preview-msg gsd-explorer-error-text"
      });
    } else if (!this.previewIsText) {
      const binWrap = content.createDiv({
        cls: "gsd-explorer-preview-center"
      });
      binWrap.createSpan({
        text: `Binary file (${this.previewMimeType ?? "unknown"}) \u2014 ${humanSize(this.previewFileSize)}`,
        cls: "gsd-explorer-preview-msg"
      });
    } else if (this.previewContent) {
      content.createEl("pre", {
        text: this.previewContent,
        cls: "gsd-explorer-preview-code"
      });
    } else {
      const emptyWrap = content.createDiv({
        cls: "gsd-explorer-preview-center"
      });
      emptyWrap.createSpan({
        text: "Empty file",
        cls: "gsd-explorer-preview-msg"
      });
    }
...
// src/TerminalPicker.ts
var import_obsidian5 = require("obsidian");

// New: Full-featured workspace/project/session browser modal
var TerminalPicker = class extends import_obsidian5.Modal {
  constructor(app, plugin, mode = "terminal") {
    super(app);
    this.plugin = plugin;
    this.mode = mode;
    this.sessionsByWs = {}; // cached tmux sessions per workspace
    this.sessionLoadErrors = [];
    this.activeTab = "projects"; // "projects" | "sessions" | "categories"
    this.sessionsLoaded = false;
    this.sessionLoadSeq = 0;
  }

  isDefaultVisibleWorkspace(ws) {
    if (!ws) return false;
    const name = String(ws.coderName || ws.name || "").trim().toLowerCase();
    const scope = String(ws.scope || "").trim().toLowerCase();
    const type = String(ws.type || "").trim().toLowerCase();
  ...
  getDefaultVisibleWorkspaces(workspaces) {
    return (workspaces || []).filter((ws) => this.isDefaultVisibleWorkspace(ws));
  }

  getConfiguredWorkspaces() {
    const topLevel = this.plugin.settings.workspaces;
    const nested = this.plugin.settings.gsd?.workspaces;
    if (Array.isArray(topLevel) && topLevel.length > 0) return this.getDefaultVisibleWorkspaces(topLevel);
    if (Array.isArray(nested) && nested.length > 0) return this.getDefaultVisibleWorkspaces(nested);
    if (Array.isArray(topLevel)) return this.getDefaultVisibleWorkspaces(topLevel);
    if (Array.isArray(nested)) return this.getDefaultVisibleWorkspaces(nested);
    return [];
  }

  getCoderUser() {
    return this.plugin.settings.coderUser || this.plugin.settings.gsd?.coderUser || "";
  }

  isCurrentCoderWorkspace(wsConfig) {
    if (!wsConfig || (wsConfig.type || "coder") !== "coder") return false;
    ...
    const configuredOwner = this.getCoderUser();
    if (!currentWorkspace || wsConfig.coderName !== currentWorkspace) return false;
  ...
  getCategories() {
    this.plugin.settings.terminalCategories = this.normalizeCategories(this.plugin.settings.terminalCategories || []);
    return this.plugin.settings.terminalCategories;
  }

  getTargetCategory() {
    const category = this.normalizeCategoryName(this.plugin.settings.terminalDefaultCategory || "");
    return this.getCategories().includes(category) ? category : "";
  }

  isSystemCategoryName(category) {
    return String(category || "").trim().toLowerCase() === "system";
  }

  getSystemCategoryName() {
    const existing = this.getCategories().find((category) => this.isSystemCategoryName(category));
    if (existing) return existing;
    this.plugin.settings.terminalCategories.push("System");
    this.plugin.saveSettings();
    this.syncOpenTerminalCategories();
    return "System";
  }

  async ensureSystemCategory() {
    const systemCategory = this.getSystemCategoryName();
    await this.saveCategorySettings();
    return systemCategory;
  }

  getSessionCategoryMap() {
    if (!this.plugin.settings.terminalSessionCategories || typeof this.plugin.settings.terminalSessionCategories !== "object" || Array.isArray(this.plugin.settings.terminalSessionCategories)) {
      this.plugin.settings.terminalSessionCategories = {};
    }
    return this.plugin.settings.terminalSessionCategories;
  }

  getSessionCategoryKey(workspaceName, sessionName) {
    const name = String(sessionName || "").trim();
    if (!name) return "";
    return `${String(workspaceName || "*").trim() || "*"}::${name}`;
  }

  getStoredSessionCategory(workspaceName, sessionName) {
    const map = this.getSessionCategoryMap();
    const key = this.getSessionCategoryKey(workspaceName, sessionName);
    if (key && Object.prototype.hasOwnProperty.call(map, key)) return this.normalizeCategoryName(map[key]);
    const fallbackKey = this.getSessionCategoryKey("*", sessionName);
    if (fallbackKey && Object.prototype.hasOwnProperty.call(map, fallbackKey)) return this.normalizeCategoryName(map[fallbackKey]);
    return null;
  }

  resolveSessionOpenCategory(workspaceName, sessionName) {
    const systemInfo = this.getSystemSessionInfo(sessionName);
    if (systemInfo) {
      const systemCategory = this.getSystemCategoryName();
      const key = this.getSessionCategoryKey(workspaceName, sessionName);
      if (key && this.getSessionCategoryMap()[key] !== systemCategory) {
        this.getSessionCategoryMap()[key] = systemCategory;
        this.plugin.saveSettings();
      }
      return systemCategory;
    }
    const stored = this.getStoredSessionCategory(workspaceName, sessionName);
    if (stored !== null) {
      return this.getCategories().includes(stored) ? stored : "";
    }
    return "";
  }

  isSystemSession(workspaceName, sessionName) {
    if (this.getSystemSessionInfo(sessionName)) return true;
    return this.isSystemCategoryName(this.getStoredSessionCategory(workspaceName, sessionName));
  }

  async setStoredSessionCategory(workspaceName, sessionName, category) {
    const key = this.getSessionCategoryKey(workspaceName, sessionName);
    if (!key) return;
    let nextCategory = this.normalizeCategoryName(category);
    if (this.isSystemCategoryName(nextCategory)) {
      nextCategory = await this.ensureSystemCategory();
    } else {
      nextCategory = this.getCategories().includes(nextCategory) ? nextCategory : "";
    }
    this.getSessionCategoryMap()[key] = nextCategory;
    await this.plugin.saveSettings();
    this.syncOpenSessionCategory(workspaceName, sessionName, nextCategory);
  }

  syncOpenSessionCategory(workspaceName, sessionName, category) {
    for (const leaf of this.app.workspace.getLeavesOfType("vin-terminal-view")) {
      const view = leaf.view;
      if (!view) continue;
      let changed = false;
      for (const session of view.sessions || []) {
        const matchesWorkspace = !session.__cwWorkspace || session.__cwWorkspace === workspaceName || workspaceName === "*";
        const tmuxName = session.__cwTmuxName || session.name;
        if (matchesWorkspace && tmuxName === sessionName) {
          session.category = category || "";
          changed = true;
        }
      }
      if (changed) {
        if (view.activeCategory && view.activeCategory !== "all" && view.activeSession && !view.isSessionVisibleInActiveCategory?.(view.activeSession)) {
  ...
  showSessionCategoryMenu(workspaceName, sessionName, event, onChange = null) {
    event?.preventDefault();
    event?.stopPropagation();
    const current = this.resolveSessionOpenCategory(workspaceName, sessionName);
    const menu = new import_obsidian5.Menu();
    const addChoice = (label, category) => {
      menu.addItem((item) => {
        item.setTitle(`${current === category ? "✓ " : ""}${label}`).setIcon(current === category ? "check" : "folder").onClick(async () => {
          await this.setStoredSessionCategory(workspaceName, sessionName, category);
    ...
    if (!this.getCategories().some((category) => this.isSystemCategoryName(category))) {
      addChoice("System", "System");
    }
    menu.addSeparator();
    menu.addItem((item) => {
      item.setTitle("New category").setIcon("plus").onClick(() => {
        this.openCategoryNameModal("", async (name) => {
          if (!name || this.isReservedCategoryName(name)) {
            new import_obsidian5.Notice("Use a different category name");
            return;
          }
          const existing = this.getCategories().find((c) => c.toLowerCase() === name.toLowerCase());
          const finalName = existing || name;
          if (!existing) this.plugin.settings.terminalCategories.push(finalName);
          await this.setStoredSessionCategory(workspaceName, sessionName, finalName);
          await this.saveCategorySettings();
          if (onChange) onChange();
        });
      });
  ...
  async saveCategorySettings() {
    this.plugin.settings.terminalCategories = this.normalizeCategories(this.plugin.settings.terminalCategories || []);
    if (!this.plugin.settings.terminalCategories.includes(this.plugin.settings.terminalDefaultCategory || "")) {
      this.plugin.settings.terminalDefaultCategory = "";
    }
    await this.plugin.saveSettings();
    this.syncOpenTerminalCategories();
  }

  async moveCategory(draggedCategory, targetCategory, placeAfter = false) {
    const categories = this.getCategories();
    const from = categories.indexOf(draggedCategory);
    const target = categories.indexOf(targetCategory);
    if (from < 0 || target < 0 || from === target) return;
    const [moved] = categories.splice(from, 1);
    let to = target + (placeAfter ? 1 : 0);
    if (from < to) to -= 1;
    categories.splice(Math.max(0, Math.min(to, categories.length)), 0, moved);
    this.plugin.settings.terminalCategories = categories;
    await this.saveCategorySettings();
  }

  syncOpenTerminalCategories() {
    const categories = this.getCategories();
    for (const leaf of this.app.workspace.getLeavesOfType("vin-terminal-view")) {
      const view = leaf.view;
      if (!view) continue;
      view.categories = categories.slice();
      for (const session of view.sessions || []) {
        if (session?.category && !categories.includes(session.category)) {
          session.category = "";
        }
      }
      if (view.activeCategory && view.activeCategory !== "all" && !categories.includes(view.activeCategory)) {
        view.activeCategory = "";
      }
      if (typeof view.renderTabs === "function") view.renderTabs();
      if (typeof view.saveState === "function") view.saveState();
  ...
  setTargetCategory(category) {
    ...
    this.plugin.settings.terminalDefaultCategory = this.getCategories().includes(normalized) ? normalized : "";
    this.plugin.saveSettings();
  }

  openCategoryNameModal(currentName = "", onSubmit) {
    new PickerCategoryNameModal(this.app, {
  ...
  openCategoryDeleteModal(name, onConfirm) {
    new PickerCategoryConfirmModal(this.app, {
  ...
  createCategory(bodyEl = null) {
    this.openCategoryNameModal("", async (name) => {
      ...
      const existing = this.getCategories().find((c) => c.toLowerCase() === name.toLowerCase());
      if (existing) {
        this.plugin.settings.terminalDefaultCategory = existing;
      } else {
        this.plugin.settings.terminalCategories.push(name);
        this.plugin.settings.terminalDefaultCategory = name;
      }
      await this.saveCategorySettings();
      if (bodyEl) this.renderActiveBody(bodyEl);
    });
  }

  renderActiveBody(bodyEl) {
    bodyEl.empty();
    if (this.activeTab === "categories") {
      this.renderCategoriesTab(bodyEl);
    } else if (this.activeTab === "sessions") {
      this.renderSessionList(bodyEl);
    } else {
      this.renderProjectList(bodyEl);
    }
  }

  renameCategory(oldName, bodyEl = null) {
    this.openCategoryNameModal(oldName, async (nextName) => {
      if (!nextName || nextName === oldName) return;
      if (this.isReservedCategoryName(nextName) || this.getCategories().some((c) => c !== oldName && c.toLowerCase() === nextName.toLowerCase())) {
        new import_obsidian5.Notice("Use a unique category name");
        return;
      }
      this.plugin.settings.terminalCategories = this.getCategories().map((c) => c === oldName ? nextName : c);
      if (this.plugin.settings.terminalDefaultCategory === oldName) {
        this.plugin.settings.terminalDefaultCategory = nextName;
      }
      const map = this.getSessionCategoryMap();
      for (const key of Object.keys(map)) {
        if (map[key] === oldName) map[key] = nextName;
      }
      for (const leaf of this.app.workspace.getLeavesOfType("vin-terminal-view")) {
        for (const session of leaf.view?.sessions || []) {
          if (session.category === oldName) session.category = nextName;
        }
        if (leaf.view?.activeCategory === oldName) leaf.view.activeCategory = nextName;
      }
      await this.saveCategorySettings();
      if (bodyEl) this.renderCategoriesTab(bodyEl);
    });
  }

  deleteCategory(name, bodyEl = null) {
    this.openCategoryDeleteModal(name, async () => {
      this.plugin.settings.terminalCategories = this.getCategories().filter((c) => c !== name);
      if (this.plugin.settings.terminalDefaultCategory === name) {
        this.plugin.settings.terminalDefaultCategory = "";
      }
      const map = this.getSessionCategoryMap();
      for (const key of Object.keys(map)) {
        if (map[key] === name) map[key] = "";
      }
      for (const leaf of this.app.workspace.getLeavesOfType("vin-terminal-view")) {
        for (const session of leaf.view?.sessions || []) {
          if (session.category === name) session.category = "";
        }
        if (leaf.view?.activeCategory === name) leaf.view.activeCategory = "";
      }
      await this.saveCategorySettings();
      if (bodyEl) this.renderCategoriesTab(bodyEl);
    });
  ...
  formatSessionLoadError(error) {
    const raw = error?.message || String(error || "Unknown error");
    const lines = raw.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
    const useful = lines.find((line) =>
      /could not resolve hostname|permission denied|connection timed out|connection refused|no route to host|name or service not known/i.test(line)
    );
    return useful || lines[0] || "Unknown error";
  }

  addSessionLoadError(workspaceName, message, loadSeq = null) {
    if (loadSeq !== null && loadSeq !== this.sessionLoadSeq) return;
    const normalizedWorkspace = String(workspaceName || "").trim();
    const normalizedMessage = String(message || "Unknown error").trim() || "Unknown error";
    const existing = this.sessionLoadErrors.find((err) => err.workspaceName === normalizedWorkspace);
    if (existing) {
      existing.message = normalizedMessage;
      return;
    }
    this.sessionLoadErrors.push({
      workspaceName: normalizedWorkspace,
      message: normalizedMessage
    });
  }

  getLocalTmuxSessions(workspaceName = "__cw_local_tmux__") {
    try {
      const { execSync } = require("child_process");
      const raw = execSync("tmux list-sessions -F '#{session_name}|||#{session_attached}|||#{@nomarh_scope}' 2>/dev/null || true", { timeout: 5000 }).toString();
      const sessions = [];
      for (const line of raw.split("\n")) {
        if (!line.trim()) continue;
        const [name, attached, scope] = line.split("|||");
        const attachedCount = Number.parseInt((attached || "0").trim(), 10) || 0;
  ...
  getOpenWorkbenchSessions() {
    ...
    for (const leaf of this.app.workspace.getLeavesOfType("vin-terminal-view")) {
      ...
      for (const session of sessions) {
        if (!session) continue;
        const workspace = session.__cwWorkspace || "";
        const tmuxName = session.__cwTmuxName || session.name || "";
  ...
  isOpenInWorkbench(openSessions, workspaceName, tmuxName) {
    return openSessions.has(`${workspaceName}\0${tmuxName}`) || openSessions.has(`*\0${tmuxName}`);
  }

  isGeneratedWorkbenchSessionName(sessionName) {
    return String(sessionName || "").startsWith("gsd-term-");
  }

  getGeneratedCleanupEntries() {
    const openWorkbenchSessions = this.getOpenWorkbenchSessions();
    return this.getLoadedSessionEntries().filter(({ ws, session, systemInfo }) => {
      if (systemInfo || this.isSystemSession(ws.coderName, session.name)) return false;
      if (!this.isGeneratedWorkbenchSessionName(session.name)) return false;
      return !this.isOpenInWorkbench(openWorkbenchSessions, ws.coderName, session.name);
    });
  }

  async runTmuxCommandForWorkspace(wsConfig, command) {
    if (!wsConfig) throw new Error("Workspace not found");
    if (wsConfig.syntheticLocal || wsConfig.coderName === "__cw_local_tmux__" || wsConfig.type === "local" || this.isCurrentCoderWorkspace(wsConfig)) {
      require("child_process").execSync(command, { timeout: 15000 });
      return;
    }
    let host, keyPath;
    if (wsConfig.type === "ssh" || wsConfig.type === "vm") {
      if (!wsConfig.sshHost) throw new Error("SSH host not set");
      host = wsConfig.sshHost;
      keyPath = wsConfig.pemPath || null;
    } else {
      const coderUser = this.getCoderUser();
      if (!coderUser) throw new Error("Coder username not set");
      host = `main.${wsConfig.coderName}.${coderUser}.coder`;
      keyPath = null;
    }
    await execInWorkspace(host, command, keyPath);
  }
  ...
  getSystemSessionInfo(sessionName, scope = "") {
    ...
    if (normalizedScope === "system") {
      return { label: "system", detail: "tmux @nomarh_scope=system" };
    }
    const exact = new Map([
      ["can-ops-refresh-loop", "Nomarh operations refresh"],
      ["claudeclaw", "Telegram bridge"],
      ["claudeclaw-main", "Telegram bridge"],
      ["bot", "Discord system watcher"],
      ...
      ["obsidian-vnc", "Obsidian VNC"],
      ["salad-orch-v3", "Salad orchestrator"],
      ["salad-orch-v3-tunnel", "Salad orchestrator tunnel"],
      ["ttyd", "Workspace tmux web terminal"],
      ["websockify", "Obsidian VNC"],
      ["x11vnc", "Obsidian VNC"],
      ["vault-github-backup", "Vault backup"],
      ["vault-backup", "Vault backup"]
    ]);
    if (exact.has(name)) return { label: "system", detail: exact.get(name) };
    if (name.startsWith("sys-")) return { label: "system", detail: "System-scoped tmux session" };
    if (name.startsWith("claudeclaw-")) return { label: "system", detail: "ClaudeClaw service" };
    if (name.startsWith("openclaw-")) return { label: "system", detail: "OpenClaw service" };
    if (name.includes("novnc") || name.includes("x11vnc")) return { label: "system", detail: "Obsidian VNC" };
    return null;
  }
  ...
  sortSessionsForDisplay(sessions) {
    return sessions
      .map((session, index) => ({
        session,
        index,
        systemInfo: this.getSystemSessionInfo(session.name, session.scope) || (this.isSystemSession(session.workspace || "", session.name) ? { label: "system", detail: "System category" } : null)
      }))
      .sort((a, b) => {
        const aSystem = !!a.systemInfo;
        const bSystem = !!b.systemInfo;
  ...
  getVisibleUserSessions(ws, sessions) {
    return this.sortSessionsForDisplay(sessions).filter(({ session, systemInfo }) =>
      !systemInfo && !this.isSystemSession(ws.coderName, session.name)
    );
  }

  async openLocalTmuxSession(sessionName, category = this.getTargetCategory(), workspaceName = "__cw_local_tmux__") {
    if (category || this.getStoredSessionCategory(workspaceName, sessionName) !== null) {
      await this.setStoredSessionCategory(workspaceName, sessionName, category);
    }
    ...
    if (leaves.length === 0) {
      new import_obsidian5.Notice("Terminal not available");
      return;
    }
    const view = leaves[0].view;
    this.app.workspace.revealLeaf(leaves[0]);
    let targetSession = null;
    if (typeof view.createSession === "function") {
      targetSession = view.createSession(sessionName, category) || view.activeSession || null;
    }
    if (targetSession && sessionName) targetSession.name = sessionName;
    return await new Promise((resolve) => {
      setTimeout(() => {
        const session = targetSession || view.activeSession;
        if (session && session.process && session.process.stdin) {
          session.__cwWorkspace = workspaceName;
          session.__cwProject = "";
          session.__cwTmuxName = sessionName;
          session.category = category || "";
          session.name = sessionName;
          if (typeof view.renderTabs === "function") view.renderTabs();
          session.process.stdin.write(`tmux set-window-option -g window-size latest 2>/dev/null; tmux new-session -A -s ${this.shellQuote(sessionName)}\n`);
          resolve(true);
        } else {
          new import_obsidian5.Notice("Local: could not attach tmux session");
          resolve(false);
        }
      }, 500);
    });
  ...
  async onOpen() {
    ...
    titleEl.setText("");
    contentEl.empty();
    contentEl.addClass("gsd-picker-modal");

    // Custom header
    const header = contentEl.createDiv({ cls: "gsd-picker-header" });
    header.createEl("h2", { text: "Connections", cls: "gsd-picker-title" });
    const subtitle = header.createEl("div", { cls: "gsd-picker-subtitle" });
    this.subtitleEl = subtitle;
    this.updatePickerSubtitle();

    // Inject modal-specific styles
    this.injectStyles();

    // Tab bar
    const tabBar = contentEl.createDiv({ cls: "gsd-tab-bar" });
    const tabProjects = tabBar.createEl("button", { text: "Sessions", cls: "gsd-tab active" });
    const tabSessions = tabBar.createEl("button", { text: "Running", cls: "gsd-tab" });
    this.runningTabEl = tabSessions;
    const tabCategories = tabBar.createEl("button", { text: "Categories", cls: "gsd-tab" });

    const body = contentEl.createDiv({ cls: "gsd-picker-body" });

    const activateTab = (tab) => {
      for (const el of [tabProjects, tabSessions, tabCategories]) {
        el.toggleClass("active", el === tab);
      }
    };
    const showProjects = () => {
      this.activeTab = "projects";
      activateTab(tabProjects);
      body.empty();
      this.renderProjectList(body);
    };
    const showSessions = async () => {
      this.activeTab = "sessions";
      activateTab(tabSessions);
      body.empty();
      body.createEl("div", { text: "Loading sessions...", cls: "gsd-loading" });
      const loaded = await this.loadAllSessions();
      if (!loaded || this.activeTab !== "sessions") return;
      this.updateSessionCountUI();
      body.empty();
      this.renderSessionList(body);
    };
    const showCategories = () => {
      this.activeTab = "categories";
      activateTab(tabCategories);
      body.empty();
      this.renderCategoriesTab(body);
    };

    tabProjects.addEventListener("click", showProjects);
    tabSessions.addEventListener("click", showSessions);
    tabCategories.addEventListener("click", showCategories);

    showProjects();
  }

  injectStyles() {
    // Remove any existing style element so hot-reload picks up new styles
    const existing = document.getElementById("gsd-picker-modal-styles");
    ...
    style.textContent = `
      ...
      /* Tab bar — Obsidian setting-tab style */
      .gsd-tab-bar {
        display: flex;
        gap: 8px;
        margin: 0 -20px 0;
      ...
      .gsd-category-chip:hover,
      .gsd-category-add:hover {
        color: var(--text-normal);
        background: var(--background-modifier-hover);
      }
      .gsd-category-chip.is-active {
        color: var(--interactive-accent);
        border-color: var(--interactive-accent);
        background: rgba(var(--interactive-accent-rgb, 0, 122, 255), 0.08);
      }
      .gsd-category-add {
        width: 26px;
        padding: 0;
      ...
      .gsd-category-manager-title {
        color: var(--text-normal);
        font-size: var(--font-ui-small);
        font-weight: 600;
      }
      .gsd-category-manager-row {
        display: grid;
      ...
      .gsd-category-manager-row.is-dragging {
        opacity: 0.55;
      }
      .gsd-category-manager-row.is-drop-before {
        box-shadow: 0 -3px 0 var(--interactive-accent);
      }
      .gsd-category-manager-row.is-drop-after {
        box-shadow: 0 3px 0 var(--interactive-accent);
      }
      .gsd-category-manager-name {
        color: var(--text-normal);
        font-size: var(--font-ui-small);
        overflow: hidden;
      ...
      .gsd-category-btn:hover {
        color: var(--text-normal);
        background: var(--background-modifier-hover);
      }
      .gsd-category-btn.is-danger:hover {
      ...
      .gsd-session-item:hover {
        background: var(--background-modifier-hover);
      }
      .gsd-session-item.is-system {
        background: rgba(220, 38, 38, 0.04);
      }
      .gsd-session-item.is-system:hover {
      ...
      .gsd-session-item.is-system .gsd-session-name {
      ...
      .gsd-session-bulk-actions > button {
        flex: 0 0 auto;
      }
      .gsd-session-bulk-note {
        color: var(--text-faint);
        font-size: 10px;
        margin-left: auto;
        max-width: 100%;
      ...
      .gsd-session-category:hover {
        color: var(--text-normal);
        background: var(--background-modifier-hover);
      }
      .gsd-session-category.is-unsorted {
        color: var(--text-faint);
      }
      .gsd-session-category.is-system {
        color: var(--text-error);
        background: rgba(220, 38, 38, 0.12);
      }
      .gsd-session-category.is-locked {
        cursor: default;
      }
      .gsd-session-category.is-locked:hover {
        color: var(--text-error);
        background: rgba(220, 38, 38, 0.12);
      }
      .gsd-session-meta {
      ...
      .gsd-session-meta.is-open-here {
        background: var(--interactive-accent);
        color: var(--text-on-accent);
      }
      .gsd-session-meta.is-system {
        background: var(--text-error);
        color: var(--text-on-accent);
      }
      .gsd-session-meta.is-detached {
        display: none;
      }

      /* Kill button — Obsidian danger button */
      .gsd-kill-btn {
        background: transparent;
        border: 0;
        color: var(--text-muted);
  ...
  renderOpenCategorySelector(container) {
    const wrap = container.createDiv({ cls: "gsd-open-category" });
    wrap.createSpan({ cls: "gsd-open-category-label", text: "Open in" });
    const row = wrap.createDiv({ cls: "gsd-category-chip-row" });
    const current = this.getTargetCategory();
    const addChip = (label, category) => {
      const chip = row.createEl("button", { cls: "gsd-category-chip", text: label });
      if (current === category) chip.addClass("is-active");
      chip.addEventListener("click", async () => {
        this.setTargetCategory(category);
        container.empty();
    ...
    const addButton = wrap.createEl("button", { cls: "gsd-category-add", text: "+", attr: { title: "New category" } });
    addButton.addEventListener("click", () => this.createCategory(container));
  }

  renderCategoriesTab(container) {
    container.empty();
    const header = container.createDiv({ cls: "gsd-category-manager-header" });
    header.createDiv({ cls: "gsd-category-manager-title", text: "Terminal categories" });
    const add = header.createEl("button", { cls: "gsd-category-btn", text: "New category" });
    add.addEventListener("click", () => this.createCategory(container));
    const categories = this.getCategories();
    if (categories.length === 0) {
      container.createEl("div", { text: "No categories yet. New sessions open in Unsorted.", cls: "gsd-empty" });
      return;
    }
    for (const category of categories) {
      const row = container.createDiv({ cls: "gsd-category-manager-row" });
      row.draggable = true;
      row.dataset.category = category;
      row.addEventListener("dragstart", (evt) => {
        this.draggingPickerCategory = category;
        evt.dataTransfer?.setData("application/x-gsd-category", category);
        if (evt.dataTransfer) evt.dataTransfer.effectAllowed = "move";
        requestAnimationFrame(() => row.addClass("is-dragging"));
      });
      row.addEventListener("dragover", (evt) => {
        const draggedCategory = evt.dataTransfer?.getData("application/x-gsd-category") || this.draggingPickerCategory;
      ...
      row.addEventListener("drop", async (evt) => {
        ...
        const rect = row.getBoundingClientRect();
        const after = evt.clientY > rect.top + rect.height / 2;
        await this.moveCategory(draggedCategory, category, after);
        this.renderCategoriesTab(container);
      });
      row.addEventListener("dragend", () => {
        this.draggingPickerCategory = null;
        row.removeClass("is-dragging");
        row.removeClass("is-drop-before");
        row.removeClass("is-drop-after");
      });
      row.createDiv({ cls: "gsd-category-manager-name", text: category });
      const actions = row.createDiv({ cls: "gsd-category-manager-actions" });
      const useBtn = actions.createEl("button", { cls: "gsd-category-btn", text: this.getTargetCategory() === category ? "Default" : "Use" });
      useBtn.disabled = this.getTargetCategory() === category;
      useBtn.addEventListener("click", async () => {
        this.setTargetCategory(category);
        await this.saveCategorySettings();
        this.renderCategoriesTab(container);
      });
      const renameBtn = actions.createEl("button", { cls: "gsd-category-btn", text: "Rename" });
      renameBtn.addEventListener("click", () => this.renameCategory(category, container));
      const deleteBtn = actions.createEl("button", { cls: "gsd-category-btn is-danger", text: "Delete" });
      deleteBtn.addEventListener("click", () => this.deleteCategory(category, container));
    }
  }

  renderProjectList(container) {
    this.renderOpenCategorySelector(container);
    const workspaces = this.getConfiguredWorkspaces();
    if (workspaces.length === 0) {
      container.createEl("div", { text: "No workspaces configured", cls: "gsd-empty" });
      return;
    }

    for (const ws of workspaces) {
      const wsGroup = container.createDiv({ cls: "gsd-ws-group" });
      wsGroup.createEl("div", {
        text: (ws.displayName || ws.coderName).toUpperCase(),
        cls: "gsd-ws-header"
      });

      if ((ws.projects || []).length === 0) {
        const item = wsGroup.createDiv({ cls: "gsd-proj-item" });
        item.createEl("div", { text: "(workspace root)", cls: "gsd-proj-name" });
        this.attachProjectHandlers(item, ws.coderName, "");
        continue;
      }
      ...
      for (const proj of ws.projects || []) {
        if (proj && proj.type === "separator") {
          wsGroup.createDiv({
            cls: "gsd-proj-separator",
            text: proj.label || proj.displayName || "Group"
          });
          continue;
        }
        const item = wsGroup.createDiv({ cls: "gsd-proj-item" });
        const label = proj.displayName || proj.path || "main";
        item.createEl("div", {
          text: label,
          cls: "gsd-proj-name"
        });
        if (proj.path && proj.path !== label) {
          item.createEl("div", {
            text: proj.path,
            cls: "gsd-proj-path"
          });
        }
        this.attachProjectHandlers(item, ws.coderName, proj.path);
      }
    }
  }

  attachProjectHandlers(itemEl, workspaceName, projectPath) {
    // Left click: find matching sessions, attach if one, pick if many, create if none
    itemEl.addEventListener("click", async (evt) => {
      evt.preventDefault();
      await this.onProjectClick(workspaceName, projectPath);
    });
    ...
    itemEl.addEventListener("contextmenu", (evt) => {
      ...
      const menu = new import_obsidian5.Menu();
      menu.addItem((item) => {
        item.setTitle("Open existing tmux session")
          .setIcon("terminal")
          .onClick(() => this.onProjectClick(workspaceName, projectPath));
      });
      menu.addItem((item) => {
        item.setTitle("Create new tmux session here")
          .setIcon("plus")
          .onClick(() => this.createNewSession(workspaceName, projectPath));
      });
      menu.showAtMouseEvent(evt);
    });
  }

  async onProjectClick(workspaceName, projectPath) {
    // Find existing sessions for this project
    const sessions = await this.getSessionsForProject(workspaceName, projectPath);

    if (sessions.length === 0) {
      // Create new session
      await this.createNewSession(workspaceName, projectPath);
    } else if (sessions.length === 1) {
      // Attach to the only one
      this.close();
      await this.plugin.openSpecificSession(workspaceName, sessions[0].name, "", false, this.resolveSessionOpenCategory(workspaceName, sessions[0].name));
    } else {
      // Multiple sessions — ask which
      const menu = new import_obsidian5.Menu();
      for (const s of sessions) {
        menu.addItem((item) => {
          item.setTitle(s.name)
            .onClick(async () => {
              this.close();
              await this.plugin.openSpecificSession(workspaceName, s.name, "", false, this.resolveSessionOpenCategory(workspaceName, s.name));
            });
        });
      }
      menu.addSeparator();
      ...
      // Show menu at center — we need a mouse event or position
      menu.showAtPosition({ x: window.innerWidth / 2, y: window.innerHeight / 2 });
    }
  }

  async createNewSession(workspaceName, projectPath) {
    this.close();
    const ts = new Date().toISOString().replace(/[:T]/g, "-").split(".")[0];
    const location = String(projectPath || workspaceName || "main").split("/").filter(Boolean).pop() || "main";
    const base = location
      .toLowerCase()
      .replace(/[^a-z0-9_.-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-+|-+$/g, "") || "main";
    const sessionName = `${base}-${ts}`;
    await this.plugin.openSpecificSession(workspaceName, sessionName, projectPath, true, "");
  }

  async getSessionsForProject(workspaceName, projectPath) {
    const all = await this.getSessionsForWorkspace(workspaceName);
    const prefix = `gsd-term-${(projectPath || workspaceName).replace(/[^a-zA-Z0-9_.\/-]/g, "_")}`;
    const locationPrefix = (String(projectPath || workspaceName || "main").split("/").filter(Boolean).pop() || "main")
      .toLowerCase()
      .replace(/[^a-z0-9_.-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-+|-+$/g, "") || "main";
    const leafName = (projectPath || "").split("/").filter(Boolean).pop();
    return all.filter(s =>
      s.name === prefix ||
      s.name.startsWith(prefix + "-") ||
      s.name === locationPrefix ||
      s.name.startsWith(locationPrefix + "-") ||
      (projectPath && s.name === projectPath) ||
      (leafName && s.name === leafName)
    );
  }

  async getSessionsForWorkspace(workspaceName, options = {}) {
    const cacheResults = options.cache !== false;
    const loadSeq = options.loadSeq ?? null;
    if (cacheResults && this.sessionsByWs[workspaceName]) return this.sessionsByWs[workspaceName];
    if (workspaceName === "__cw_local_tmux__") {
      const sessions = this.getLocalTmuxSessions(workspaceName);
      if (cacheResults) this.sessionsByWs[workspaceName] = sessions;
      return sessions;
    }
    const workspaces = this.getConfiguredWorkspaces();
    const wsConfig = workspaces.find(w => w.coderName === workspaceName);
    if (!wsConfig) return [];
    if (this.isCurrentCoderWorkspace(wsConfig)) {
      const sessions = this.getLocalTmuxSessions(workspaceName);
      if (cacheResults) this.sessionsByWs[workspaceName] = sessions;
      return sessions;
    }
    // Dispatch SSH host by workspace type
    let host, keyPath;
    if (wsConfig.type === "ssh" || wsConfig.type === "vm") {
      if (!wsConfig.sshHost) return [];
      host = wsConfig.sshHost;
      keyPath = wsConfig.pemPath || null;
    } else if (wsConfig.type === "local") {
      // Local: run tmux ls directly, no SSH
      const sessions = this.getLocalTmuxSessions(workspaceName);
      if (cacheResults) this.sessionsByWs[workspaceName] = sessions;
      return sessions;
    } else {
    ...
    try {
      const raw = await execInWorkspace(host, "tmux list-sessions -F '#{session_name}|||#{session_attached}|||#{@nomarh_scope}' 2>/dev/null || true", keyPath);
      const sessions = [];
      for (const line of raw.split("\n")) {
        if (!line.trim()) continue;
        const [name, attached, scope] = line.split("|||");
      ...
      const message = this.formatSessionLoadError(e);
      this.addSessionLoadError(workspaceName, message, loadSeq);
      console.warn(`[Workbench] Failed to list tmux sessions for ${workspaceName}: ${message}`);
      return [];
    }
  }
  ...
  async loadAllSessions() {
    ...
    this.sessionsByWs = {}; // force refresh
    this.sessionLoadErrors = [];
    this.sessionsLoaded = false;
    const workspaces = this.getConfiguredWorkspaces();
    const withTimeout = (p, ms, label) => new Promise((resolve) => {
      const timer = setTimeout(() => {
        const message = `Timed out after ${Math.round(ms / 1000)}s`;
        this.addSessionLoadError(label, message, loadSeq);
        console.warn(`[Workbench] session list timeout for ${label}: ${message}`);
        resolve([]);
      }, ms);
    ...
    const loadedWorkspaces = await Promise.all(
      workspaces.map(async (ws) => ({
        ws,
        sessions: await withTimeout(this.getSessionsForWorkspace(ws.coderName, { cache: false, loadSeq }), 10000, ws.coderName)
      }))
    );
    if (loadSeq !== this.sessionLoadSeq) return false;
    for (const { ws, sessions } of loadedWorkspaces) {
      this.sessionsByWs[ws.coderName] = sessions;
    }
    if (!workspaces.some((ws) => ws.type === "local")) {
      const localSessions = this.getLocalTmuxSessions("__cw_local_tmux__");
      if (localSessions.length > 0) {
        const representedNames = new Set();
        for (const [workspaceName, sessions] of Object.entries(this.sessionsByWs)) {
          if (workspaceName === "__cw_local_tmux__") continue;
          for (const session of sessions || []) representedNames.add(session.name);
        }
        const uniqueLocalSessions = localSessions.filter((session) => !representedNames.has(session.name));
        if (uniqueLocalSessions.length > 0) {
          this.sessionsByWs.__cw_local_tmux__ = uniqueLocalSessions;
        }
      }
    }
  ...
  getSessionWorkspaceList() {
    const workspaces = [...this.getConfiguredWorkspaces()];
    if (this.sessionsByWs.__cw_local_tmux__?.length && !workspaces.some((ws) => ws.coderName === "__cw_local_tmux__")) {
      workspaces.push({
        coderName: "__cw_local_tmux__",
  ...
  getLoadedSessionEntries() {
    const entries = [];
    for (const ws of this.getSessionWorkspaceList()) {
      const sessions = this.sessionsByWs[ws.coderName] || [];
      for (const { session, systemInfo } of this.getVisibleUserSessions(ws, sessions)) {
        entries.push({ ws, session, systemInfo });
      }
    }
    return entries;
  }
  ...
  getSessionCounts() {
    const entries = this.getLoadedSessionEntries();
    const openWorkbenchSessions = this.getOpenWorkbenchSessions();
    let attachedClients = 0;
    let openCount = 0;
    let systemCount = 0;
    for (const { ws, session, systemInfo } of entries) {
      const attachedCount = Number.isFinite(session.attachedCount) ? session.attachedCount : (session.attached ? 1 : 0);
      attachedClients += attachedCount;
      if (systemInfo || this.isSystemSession(ws.coderName, session.name)) systemCount++;
      if (this.isOpenInWorkbench(openWorkbenchSessions, ws.coderName, session.name)) openCount++;
    }
    return {
      ...
      openCount,
      systemCount,
      nonSystemCount: Math.max(0, entries.length - systemCount)
    };
  }

  updateRunningCountUI() {
    if (!this.runningTabEl) return;
    const { total } = this.getSessionCounts();
    this.runningTabEl.setText(this.sessionsLoaded ? `Running (${total})` : "Running");
  }

  updatePickerSubtitle() {
    if (!this.subtitleEl) return;
    const workspaces = this.getConfiguredWorkspaces();
    const wsCount = workspaces.length;
    const projCount = workspaces.reduce((sum, w) => sum + (w.projects || []).filter((p) => p && p.type !== "separator").length, 0);
    const parts = [
      `${wsCount} connection${wsCount === 1 ? "" : "s"}`,
      `${projCount} configured session${projCount === 1 ? "" : "s"}`
    ];
    const { total, attachedClients } = this.getSessionCounts();
    if (this.sessionsLoaded) {
      parts.push(`${total} running tmux session${total === 1 ? "" : "s"}`);
      if (attachedClients > 0) parts.push(`${attachedClients} attached client${attachedClients === 1 ? "" : "s"}`);
    }
    this.subtitleEl.setText(parts.join(" • "));
  }

  updateSessionCountUI() {
    this.updateRunningCountUI();
    this.updatePickerSubtitle();
  }

  async refreshSessionList(container) {
    const loaded = await this.loadAllSessions();
    if (!loaded || this.activeTab !== "sessions" || !container?.isConnected) return;
    container.empty();
    this.renderSessionList(container);
  }

  async attachAllSessions(container) {
    const entries = this.getAttachableSessionEntries();
    if (entries.length === 0) {
      new import_obsidian5.Notice("No non-system tmux sessions to attach");
      return;
    }
    for (const { ws, session } of entries) {
      const category = this.resolveSessionOpenCategory(ws.coderName, session.name);
      if (ws.syntheticLocal || this.isCurrentCoderWorkspace(ws) || session.localTmux && ws.coderName === "__cw_local_tmux__") {
        await this.openLocalTmuxSession(session.name, category, ws.coderName);
      } else {
        await this.plugin.openSpecificSession(ws.coderName, session.name, "", false, category);
      }
    }
    new import_obsidian5.Notice(`Attached ${entries.length} tmux session${entries.length === 1 ? "" : "s"}`);
    await this.refreshSessionList(container);
  }

  getAttachableSessionEntries() {
    const openWorkbenchSessions = this.getOpenWorkbenchSessions();
    return this.getLoadedSessionEntries().filter(({ ws, session, systemInfo }) => {
      if (systemInfo || this.isSystemSession(ws.coderName, session.name)) return false;
      return !this.isOpenInWorkbench(openWorkbenchSessions, ws.coderName, session.name);
  ...
  confirmAttachAllSessions(container) {
    const count = this.getAttachableSessionEntries().length;
    if (count === 0) {
      new import_obsidian5.Notice("No non-system tmux sessions to attach");
      return;
    }
    new PickerCategoryConfirmModal(this.app, {
      title: "Attach tmux sessions",
      message: `Open ${count} non-system tmux session${count === 1 ? "" : "s"} in Workbench. System sessions and already open sessions are skipped.`,
      confirmText: `Attach ${count}`,
      onConfirm: () => this.attachAllSessions(container)
    }).open();
  }

  async detachAllSessions(container) {
    let count = 0;
    const emptiedLeaves = [];
    for (const leaf of this.app.workspace.getLeavesOfType("vin-terminal-view")) {
      const view = leaf.view;
      ...
      for (const session of [...view.sessions]) {
        ...
        if (!tmuxName) continue;
        const workspaceName = session.__cwWorkspace || "*";
        if (this.isSystemSession(workspaceName, tmuxName)) continue;
        view.closeSession(session);
        closedInLeaf++;
        count++;
      }
      if (closedInLeaf > 0 && (view.sessions || []).length === 0) {
        emptiedLeaves.push(leaf);
      }
    }
    for (const leaf of emptiedLeaves) {
      try {
        leaf.detach();
      } catch {
      }
    }
    new import_obsidian5.Notice(count === 0 ? "No attached non-system tmux sessions" : `Detached ${count} tmux session${count === 1 ? "" : "s"}`);
    await new Promise((resolve) => setTimeout(resolve, 400));
    await this.refreshSessionList(container);
  }

  confirmCleanupGeneratedSessions(container) {
    const count = this.getGeneratedCleanupEntries().length;
    if (count === 0) {
      new import_obsidian5.Notice("No old generated tmux sessions to clean");
      return;
    }
    new PickerCategoryConfirmModal(this.app, {
      title: "Clean generated tmux sessions",
      message: `Kill ${count} generated gsd-term-* tmux session${count === 1 ? "" : "s"} ${count === 1 ? "that is" : "that are"} not open in Workbench. System and manually named sessions are skipped.`,
      confirmText: `Kill ${count}`,
      onConfirm: () => this.cleanupGeneratedSessions(container)
    }).open();
  }

  async cleanupGeneratedSessions(container) {
    const entries = this.getGeneratedCleanupEntries();
    if (entries.length === 0) {
      new import_obsidian5.Notice("No old generated tmux sessions to clean");
      return;
    }
    ...
    for (const { ws, session } of entries) {
      const key = ws.coderName;
      if (!groups.has(key)) groups.set(key, { ws, names: [] });
      groups.get(key).names.push(session.name);
    }
    let cleaned = 0;
    let failed = 0;
    for (const { ws, names } of groups.values()) {
      const command = names.map((name) => `tmux kill-session -t ${this.shellQuote(name)} 2>/dev/null || true`).join("; ");
      try {
        await this.runTmuxCommandForWorkspace(ws, command);
        cleaned += names.length;
      } catch (e) {
        console.error("[Workbench] Failed to clean generated tmux sessions for", ws.coderName, e);
        failed += names.length;
      }
    }
    new import_obsidian5.Notice(failed === 0 ? `Cleaned ${cleaned} generated tmux session${cleaned === 1 ? "" : "s"}` : `Cleaned ${cleaned}, failed ${failed}`);
    await this.refreshSessionList(container);
  }

  renderSessionBulkActions(container) {
    const attachableCount = this.getAttachableSessionEntries().length;
    let detachableCount = 0;
    for (const leaf of this.app.workspace.getLeavesOfType("vin-terminal-view")) {
      const view = leaf.view;
      for (const session of view?.sessions || []) {
        const tmuxName = session.__cwTmuxName || "";
    ...
    const cleanupCount = this.getGeneratedCleanupEntries().length;
    const bar = container.createDiv({ cls: "gsd-session-bulk-actions" });
    const refreshBtn = bar.createEl("button", { cls: "gsd-category-btn", text: "Refresh" });
    refreshBtn.addEventListener("click", () => this.refreshSessionList(container));
    const attachBtn = bar.createEl("button", { cls: "gsd-category-btn", text: `Attach all${attachableCount ? ` (${attachableCount})` : ""}` });
    attachBtn.disabled = attachableCount === 0;
    attachBtn.addEventListener("click", () => this.confirmAttachAllSessions(container));
    const detachBtn = bar.createEl("button", { cls: "gsd-category-btn", text: `Detach all${detachableCount ? ` (${detachableCount})` : ""}` });
    detachBtn.disabled = detachableCount === 0;
    detachBtn.addEventListener("click", () => this.detachAllSessions(container));
    const cleanupBtn = bar.createEl("button", { cls: "gsd-category-btn is-danger", text: `Clean generated${cleanupCount ? ` (${cleanupCount})` : ""}` });
    cleanupBtn.disabled = cleanupCount === 0;
    cleanupBtn.addEventListener("click", () => this.confirmCleanupGeneratedSessions(container));
    bar.createEl("span", { cls: "gsd-session-bulk-note", text: "System sessions are skipped; cleanup only touches gsd-term-*" });
  }

  renderSessionCountSummary(container) {
    const counts = this.getSessionCounts();
    const summary = container.createDiv({ cls: "gsd-session-count-summary" });
    const addMetric = (label, value) => {
      const metric = summary.createDiv({ cls: "gsd-session-count-metric" });
      metric.createSpan({ cls: "gsd-session-count-value", text: String(value) });
      metric.createSpan({ cls: "gsd-session-count-label", text: label });
    };
    addMetric("tmux sessions", counts.total);
    addMetric("open here", counts.openCount);
    addMetric("attached clients", counts.attachedClients);
    if (counts.systemCount > 0) addMetric("system", counts.systemCount);
  }

  renderSessionLoadWarnings(container) {
    if (!this.sessionLoadErrors.length) return;
    const count = this.sessionLoadErrors.length;
    const names = this.sessionLoadErrors.map((err) => err.workspaceName).filter(Boolean).join(", ");
    container.createDiv({
      cls: "gsd-session-load-warning",
      text: `${count} configured workspace${count === 1 ? "" : "s"} unavailable${names ? `: ${names}` : ""}. Showing sessions found from available tmux sources.`,
      attr: {
        title: this.sessionLoadErrors.map((err) => `${err.workspaceName}: ${err.message}`).join("\n")
      }
    });
  }
  ...
  renderSessionList(container) {
    this.updateSessionCountUI();
    this.renderOpenCategorySelector(container);
    this.renderSessionCountSummary(container);
    this.renderSessionLoadWarnings(container);
    this.renderSessionBulkActions(container);
    const workspaces = this.getSessionWorkspaceList();
    const openWorkbenchSessions = this.getOpenWorkbenchSessions();
    let totalCount = 0;
    ...
    for (const ws of workspaces) {
      ...
      for (const { session: s, systemInfo } of visibleSessions) {
        const row = wsGroup.createDiv({ cls: `gsd-session-item${systemInfo ? " is-system" : ""}` });
        const nameEl = row.createEl("div", { cls: "gsd-session-name", text: s.name });
        const actionsEl = row.createDiv({ cls: "gsd-session-actions" });
        const isOpenHere = this.isOpenInWorkbench(openWorkbenchSessions, ws.coderName, s.name);
        if (systemInfo) {
          actionsEl.createEl("span", {
            text: "system",
            cls: "gsd-session-meta is-system",
            attr: { title: systemInfo.detail }
          });
        }
        if (isOpenHere) {
          actionsEl.createEl("span", { text: "open", cls: "gsd-session-meta is-open-here" });
        }
        const attachedCount = Number.isFinite(s.attachedCount) ? s.attachedCount : (s.attached ? 1 : 0);
        if (!isOpenHere && !systemInfo && attachedCount > 0) {
          row.title = `${attachedCount} tmux client${attachedCount === 1 ? "" : "s"} attached outside Workbench`;
          actionsEl.createEl("span", {
            text: `attached ${attachedCount}`,
            cls: "gsd-session-meta",
            attr: { title: row.title }
          });
        }
        const sessionCategory = this.resolveSessionOpenCategory(ws.coderName, s.name);
        if (systemInfo) {
          actionsEl.createEl("span", {
            text: sessionCategory || "System",
            cls: "gsd-session-category is-system is-locked",
            attr: { title: "System session category is locked" }
          });
          nameEl.title = systemInfo.detail || "System session";
        } else {
          const categoryBtn = actionsEl.createEl("button", {
            text: sessionCategory || "Unsorted",
            cls: `gsd-session-category${sessionCategory ? "" : " is-unsorted"}${this.isSystemCategoryName(sessionCategory) ? " is-system" : ""}`,
            attr: { title: "Choose category for this tmux session" }
          });
          categoryBtn.addEventListener("click", (evt) => {
            this.showSessionCategoryMenu(ws.coderName, s.name, evt, () => {
              container.empty();
              this.renderSessionList(container);
            });
          });
          ...
          nameEl.addEventListener("click", async () => {
            this.close();
            const category = this.resolveSessionOpenCategory(ws.coderName, s.name);
            if (ws.syntheticLocal || this.isCurrentCoderWorkspace(ws) || s.localTmux && ws.coderName === "__cw_local_tmux__") {
              await this.openLocalTmuxSession(s.name, category, ws.coderName);
            } else {
              await this.plugin.openSpecificSession(ws.coderName, s.name, "", false, category);
            }
          });
        }

        if (!systemInfo) {
          const killBtn = actionsEl.createEl("button", { text: "kill", cls: "gsd-kill-btn" });
          killBtn.addEventListener("click", async (evt) => {
            evt.stopPropagation();
            if (ws.syntheticLocal || s.localTmux && ws.coderName === "__cw_local_tmux__") {
              try {
                require("child_process").execSync(`tmux kill-session -t ${this.shellQuote(s.name)}`, { timeout: 5000 });
                new import_obsidian5.Notice(`Killed ${s.name}`);
              } catch (e) {
                new import_obsidian5.Notice(`Failed: ${e.message}`);
              }
            } else {
              await this.killSession(ws.coderName, s.name);
            }
            await this.refreshSessionList(container);
          });
        }
    ...
    if (totalCount === 0) {
      container.createEl("div", { text: "No tmux sessions running", cls: "gsd-empty" });
    }
  }

  async killSession(workspaceName, sessionName) {
    const workspaces = this.getConfiguredWorkspaces();
    const wsConfig = workspaces.find(w => w.coderName === workspaceName);
    if (!wsConfig) return;
    try {
      await this.runTmuxCommandForWorkspace(wsConfig, `tmux kill-session -t ${this.shellQuote(sessionName)}`);
      new import_obsidian5.Notice(`Killed ${sessionName}`);
    } catch (e) {
      new import_obsidian5.Notice(`Failed to kill ${sessionName}: ${e.message}`);
    }
  }
...
var PickerCategoryNameModal = class extends import_obsidian5.Modal {
  ...
  onOpen() {
    ...
    const submit = () => {
      ...
      if (!name) {
        new import_obsidian5.Notice("Category name is required");
        input.focus();
        return;
      }
      this.close();
...
var PickerCategoryConfirmModal = class extends import_obsidian5.Modal {
  constructor(app, options) {
    super(app);
    this.options = options;
  }
...
// main.ts
var TERMINAL_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="4 17 10 11 4 5"></polyline><line x1="12" y1="19" x2="20" y2="19"></line></svg>`;
var GSDPlugin = class extends import_obsidian6.Plugin {
  constructor() {
    super(...arguments);
    this.settings = { ...DEFAULT_SETTINGS };
    /** Path to the written PTY helper script; set after onload writes it. */
    this.ptyHelperPath = null;
  }
  async onload() {
    console.log("[Workbench] Loading Can Workbench plugin");
    await this.loadSettings();
    (0, import_obsidian6.addIcon)("gsd-terminal", TERMINAL_ICON_SVG);
    try {
      this.ptyHelperPath = writePtyHelper();
    } catch (err) {
      console.error("[Workbench] Failed to write PTY helper:", err);
    }
    this.registerView(TERMINAL_VIEW_TYPE, (leaf) => {
      return new TerminalView(leaf, this);
    });
    this.registerView(STATUS_VIEW_TYPE, (leaf) => {
      return new StatusView(leaf, this);
    });
    this.registerView(EXPLORER_VIEW_TYPE, (leaf) => {
    ...
    this.addCommand({
      id: "open-gsd-status",
      name: "Open Workbench status dashboard",
      callback: () => {
        this.openStatusView();
      }
    });
    this.addRibbonIcon("folder", "Open Workspace Explorer", () => {
      this.openExplorerView();
    });
    this.addCommand({
      id: "open-gsd-explorer",
      name: "Open Workbench file explorer",
      callback: () => {
        this.openExplorerView();
      }
    });
    this.addCommand({
      id: "open-gsd-terminal",
      name: "Open session",
      callback: () => {
    ...
    this.registerEvent(
      this.app.workspace.on("active-leaf-change", (leaf) => {
        if (leaf?.view?.getViewType() === TERMINAL_VIEW_TYPE) {
    ...
    console.log("[Workbench] Can Workbench plugin loaded");
  }
  async onunload() {
    console.log("[Workbench] Unloading Can Workbench plugin");
  ...
  getConfiguredWorkspaces() {
    const topLevel = this.settings.workspaces;
    const nested = this.settings.gsd?.workspaces;
    if (Array.isArray(topLevel) && topLevel.length > 0) return this.getDefaultVisibleWorkspaces(topLevel);
    if (Array.isArray(nested) && nested.length > 0) return this.getDefaultVisibleWorkspaces(nested);
    if (Array.isArray(topLevel)) return this.getDefaultVisibleWorkspaces(topLevel);
    if (Array.isArray(nested)) return this.getDefaultVisibleWorkspaces(nested);
  ...
  getConfiguredCoderUser() {
    return this.settings.coderUser || this.settings.gsd?.coderUser || "";
  }
  normalizeTerminalCategoryName(name) {
    return String(name || "").trim().replace(/\s+/g, " ").slice(0, 40);
  }
  getTerminalCategories() {
    this.settings.terminalCategories = Array.isArray(this.settings.terminalCategories) ? this.settings.terminalCategories : [];
    const seen = /* @__PURE__ */ new Set();
    this.settings.terminalCategories = this.settings.terminalCategories.filter((raw) => {
      const name = this.normalizeTerminalCategoryName(raw);
      if (!name || ["all", "uncategorized", "unsorted"].includes(name.toLowerCase())) return false;
      const key = name.toLowerCase();
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    }).map((name) => this.normalizeTerminalCategoryName(name));
    return this.settings.terminalCategories;
  }
  resolveTerminalCategory(category) {
    const name = this.normalizeTerminalCategoryName(category);
    return this.getTerminalCategories().includes(name) ? name : "";
  }
  ensureTerminalCategory(category) {
    const name = this.normalizeTerminalCategoryName(category);
    if (!name || ["all", "uncategorized", "unsorted"].includes(name.toLowerCase())) return "";
    const categories = this.getTerminalCategories();
    const existing = categories.find((item) => item.toLowerCase() === name.toLowerCase());
    if (existing) return existing;
    categories.push(name);
    return name;
  }
  getTerminalSessionCategoryMap() {
    if (!this.settings.terminalSessionCategories || typeof this.settings.terminalSessionCategories !== "object" || Array.isArray(this.settings.terminalSessionCategories)) {
      this.settings.terminalSessionCategories = {};
    }
    return this.settings.terminalSessionCategories;
  }
  getTerminalSessionCategorySourceMap() {
    if (!this.settings.terminalSessionCategorySources || typeof this.settings.terminalSessionCategorySources !== "object" || Array.isArray(this.settings.terminalSessionCategorySources)) {
      this.settings.terminalSessionCategorySources = {};
    }
    return this.settings.terminalSessionCategorySources;
  }
  getTerminalSessionCategoryKey(workspaceName, sessionName) {
    const name = String(sessionName || "").trim();
    if (!name) return "";
    return `${String(workspaceName || "*").trim() || "*"}::${name}`;
  }
  getTerminalSessionCategorySource(workspaceName, sessionName) {
    const sourceMap = this.getTerminalSessionCategorySourceMap();
    const key = this.getTerminalSessionCategoryKey(workspaceName, sessionName);
    if (key && Object.prototype.hasOwnProperty.call(sourceMap, key)) return sourceMap[key];
    const fallbackKey = this.getTerminalSessionCategoryKey("*", sessionName);
    if (fallbackKey && Object.prototype.hasOwnProperty.call(sourceMap, fallbackKey)) return sourceMap[fallbackKey];
    return "";
  }
  getStoredTerminalSessionCategory(workspaceName, sessionName, manualOnly = false) {
    const map = this.getTerminalSessionCategoryMap();
    const sourceMap = this.getTerminalSessionCategorySourceMap();
    const key = this.getTerminalSessionCategoryKey(workspaceName, sessionName);
    if (key && Object.prototype.hasOwnProperty.call(map, key)) {
      const source = sourceMap[key] || "manual";
      if (!manualOnly || source === "manual") return this.resolveTerminalCategory(map[key]);
    }
    const fallbackKey = this.getTerminalSessionCategoryKey("*", sessionName);
    if (fallbackKey && Object.prototype.hasOwnProperty.call(map, fallbackKey)) {
      const source = sourceMap[fallbackKey] || "manual";
      if (!manualOnly || source === "manual") return this.resolveTerminalCategory(map[fallbackKey]);
    }
    return null;
  }
  getTerminalLocationCandidates() {
    const seen = /* @__PURE__ */ new Set();
    const result = [];
    const add = (value) => {
      const slug = this.slugTerminalNamePart(value, "");
      if (!slug || seen.has(slug)) return;
      seen.add(slug);
      result.push(slug);
    };
    for (const ws of this.getConfiguredWorkspaces()) {
      add(ws?.displayName);
      add(ws?.coderName);
  ...
  getTerminalLocationCategory(sessionName) {
    const slug = this.slugTerminalNamePart(String(sessionName || "").replace(/^gsd-term-/, ""), "");
    if (!slug) return "";
    for (const candidate of this.getTerminalLocationCandidates()) {
      if (slug === candidate || slug.startsWith(`${candidate}-`)) return this.ensureTerminalCategory(candidate);
    }
    return this.ensureTerminalCategory(slug.split("-").filter(Boolean)[0] || slug);
  }
  resolveTerminalOpenCategory(workspaceName, sessionName, requestedCategory = "") {
    const requested = this.normalizeTerminalCategoryName(requestedCategory);
    if (requested && this.getTerminalCategories().includes(requested)) return requested;
    const manual = this.getStoredTerminalSessionCategory(workspaceName, sessionName, true);
    if (manual !== null) return manual;
    const auto = this.getTerminalLocationCategory(sessionName);
    if (auto) return auto;
    const stored = this.getStoredTerminalSessionCategory(workspaceName, sessionName, false);
    if (stored !== null) return stored;
    return "";
  }
  rememberTerminalSessionCategory(workspaceName, sessionName, category, source = "auto") {
    const map = this.getTerminalSessionCategoryMap();
    const sourceMap = this.getTerminalSessionCategorySourceMap();
    const key = this.getTerminalSessionCategoryKey(workspaceName, sessionName);
    if (!key) return;
    const storedSource = sourceMap[key] || (Object.prototype.hasOwnProperty.call(map, key) ? "manual" : "auto");
    if (source !== "manual" && storedSource === "manual") return;
    const nextCategory = this.resolveTerminalCategory(category);
    if (!nextCategory && !Object.prototype.hasOwnProperty.call(map, key)) return;
    map[key] = nextCategory;
    sourceMap[key] = source === "manual" ? "manual" : "auto";
    this.saveSettings();
  }
  /**
   * Open a new GSD terminal tab.
   *
   * Always creates a new leaf (tab) — never reuses an existing one.
   * If workspaceName/projectPath are not provided and multiple workspaces
   * are configured, opens the picker modal instead.
   */
  /**
   * Open internetvin-terminal attached to a specific tmux session name.
   * Creates the session if createNew is true (or if it doesn't exist).
   */
  async openSpecificSession(workspaceName, sessionName, projectPath = "", createNew = false, category = "") {
    const { workspace } = this.app;
    const wsConfig = this.getConfiguredWorkspaces().find(w => w.coderName === workspaceName);
    const coderUser = this.getConfiguredCoderUser();
    if (!wsConfig || !coderUser) {
      new (require("obsidian")).Notice("Workbench: workspace or coderUser not configured");
      return;
    }
	    const sshHost = `main.${wsConfig.coderName}.${coderUser}.coder`;
	    const resolvedProject = projectPath || wsConfig.projects[0]?.path || "";
	    const cdTarget = resolvedProject ? `~/${resolvedProject}` : "~";
	    const location = this.slugTerminalNamePart(resolvedProject || workspaceName || "main", "main");
	    const requestedName = this.slugTerminalNamePart(sessionName || location, location);
	    const effectiveSessionName = createNew && !requestedName.startsWith(`${location}-`) && requestedName !== location
	      ? `${location}-${requestedName}`
	      : requestedName;
	    const targetCategory = this.resolveTerminalOpenCategory(workspaceName, effectiveSessionName, category);
	    this.rememberTerminalSessionCategory(workspaceName, effectiveSessionName, targetCategory);

    const pluginDir = this.getPluginDir();
    const keyData = getSecret(pluginDir, SECRET_KEYS.sshKey(wsConfig.coderName));
    let keyArg = "";
    if (keyData) {
      const fs = require("fs");
      const keyPath = `/tmp/gsd-ssh-key-${wsConfig.coderName}`;
      fs.writeFileSync(keyPath, keyData + "\n", { mode: 0o600 });
      keyArg = ` -i "${keyPath}"`;
    }

    // After ensuring/creating session, make tmux follow the active Workbench client size.
	    const resizeOpts = `tmux set-window-option -t ${effectiveSessionName} window-size latest 2>/dev/null; tmux set-option -t ${effectiveSessionName} aggressive-resize off 2>/dev/null; tmux set-option -t ${effectiveSessionName} -w mouse on 2>/dev/null`;
	    const ensureCmd = createNew
	      ? `tmux new-session -d -s ${effectiveSessionName} -c ${cdTarget} 2>/dev/null; ${resizeOpts}; TERM=xterm-256color tmux attach-session -t ${effectiveSessionName}`
	      : `tmux has-session -t ${effectiveSessionName} 2>/dev/null || tmux new-session -d -s ${effectiveSessionName} -c ${cdTarget}; ${resizeOpts}; TERM=xterm-256color tmux attach-session -t ${effectiveSessionName}`;
    const sshCmd = `ssh -tt${keyArg} -o StrictHostKeyChecking=no ${sshHost} '${ensureCmd}'`;

    const VIN_VIEW = "vin-terminal-view";
    let leaves = workspace.getLeavesOfType(VIN_VIEW);
    let leaf;
    if (leaves.length === 0) {
      leaf = workspace.getLeaf("tab");
      await leaf.setViewState({ type: VIN_VIEW, active: true });
      leaves = workspace.getLeavesOfType(VIN_VIEW);
      if (leaves.length === 0) {
        new (require("obsidian")).Notice("Workbench: internetvin-terminal plugin not found");
        return;
      }
      leaf = leaves[0];
    } else {
    ...
    workspace.revealLeaf(leaf);
	    ...
	    let targetSession = null;
	    if (typeof view.createSession === "function") {
	      targetSession = view.createSession(effectiveSessionName, targetCategory) || view.activeSession || null;
	    }
	    if (targetSession && effectiveSessionName) {
	      targetSession.name = effectiveSessionName;
	      targetSession.category = targetCategory;
	      if (typeof view.renderTabs === "function") view.renderTabs();
	    }
	    ...
	    setTimeout(() => {
	      ...
	      if (session && session.process && session.process.stdin) {
	        if (effectiveSessionName) {
	          session.name = effectiveSessionName;
	          session.category = targetCategory;
	          if (typeof view.renderTabs === "function") view.renderTabs();
	        }
        session.process.stdin.write(sshCmd + "\n");
      } else {
        new (require("obsidian")).Notice("Workbench: could not send SSH command to terminal");
      }
    }, 500);
  }
  ...
  async openTerminalView(workspaceName, projectPath, category = "") {
    ...
    const { workspace } = this.app;
    // Use internetvin-terminal instead of custom gsd-terminal-view
    // Resolve workspace config and SSH host
    const configuredWorkspaces = this.getConfiguredWorkspaces();
    const wsName = workspaceName || configuredWorkspaces[0]?.coderName;
    const wsConfig = configuredWorkspaces.find(w => w.coderName === wsName);
    const coderUser = this.getConfiguredCoderUser();
    if (!wsConfig || !coderUser) {
      new (require("obsidian")).Notice("Workbench: workspace or coderUser not configured");
      return;
	    ...
	    const sshHost = `main.${wsConfig.coderName}.${coderUser}.coder`;
	    const resolvedProject = projectPath || wsConfig.projects[0]?.path || "";
	    const tmuxName = this.slugTerminalNamePart(resolvedProject || wsConfig.coderName || "main", "main");
    const cdTarget = resolvedProject ? `~/${resolvedProject}` : "~";
    const targetCategory = this.resolveTerminalOpenCategory(wsName, tmuxName, category);
    this.rememberTerminalSessionCategory(wsName, tmuxName, targetCategory);
    ...
    // Build single SSH command that ensures tmux session and attaches
    const ensureCmd = `tmux has-session -t ${tmuxName} 2>/dev/null || tmux new-session -d -s ${tmuxName} -c ${cdTarget}; tmux set-window-option -t ${tmuxName} window-size latest 2>/dev/null; tmux set-option -t ${tmuxName} -w mouse on 2>/dev/null; TERM=xterm-256color tmux attach-session -t ${tmuxName}`;
    const sshCmd = `ssh -tt${keyArg} -o StrictHostKeyChecking=no ${sshHost} '${ensureCmd}'`;

    // Find or create an internetvin-terminal view
    const VIN_VIEW = "vin-terminal-view";
    let leaves = workspace.getLeavesOfType(VIN_VIEW);
    let leaf;
    if (leaves.length === 0) {
    ...
    let targetSession = null;
    if (typeof view.createSession === "function") {
      targetSession = view.createSession(tmuxName, targetCategory) || view.activeSession || null;
    }
    if (targetSession) {
      targetSession.name = tmuxName;
      targetSession.category = targetCategory;
      if (typeof view.renderTabs === "function") view.renderTabs();
    }
  ...
  /**
   * Open or reveal the GSD file explorer as a singleton tab.
   *
   * Singleton: if an ExplorerView leaf already exists, reveals it.
   * Otherwise creates a new tab. Project selection is handled inside the
   * view via the left-panel session list — no TerminalPicker needed.
   */
  async openExplorerView() {
    const { workspace } = this.app;
    const existing = workspace.getLeavesOfType(EXPLORER_VIEW_TYPE);
  ...
  /**
   * Open or reveal the Workspace Status dashboard as a tab.
   * Singleton: if a StatusView leaf already exists, reveals it.
   * Otherwise creates a new tab (matching openTerminalView() pattern).
   */
  async openStatusView() {
    const { workspace } = this.app;
  ...
  /** Called when Obsidian switches themes — propagates to all open terminals, status, and explorer views. */
  onThemeChange() {
    const { workspace } = this.app;
    workspace.iterateAllLeaves((leaf) => {
      if (leaf.view.getViewType() === TERMINAL_VIEW_TYPE) {
        const view = leaf.view;
        if (typeof view.updateTheme === "function") {
          view.updateTheme();
        }
      }
      if (leaf.view.getViewType() === STATUS_VIEW_TYPE) {
        const view = leaf.view;
        if (typeof view.updateTheme === "function") {
          view.updateTheme();
        }
      }
      if (leaf.view.getViewType() === EXPLORER_VIEW_TYPE) {
        const view = leaf.view;
        if (typeof view.updateTheme === "function") {
          view.updateTheme();
        }

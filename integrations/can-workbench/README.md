...
- [Can Workbench v2.2.0](https://github.com/cansitki/can-workbench/releases/tag/v2.2.0), source commit `e09d8a0b8e2af60405aa05b4477850f88f8dd995`: Obsidian modal, WebCrypto encryption, Local/Coder/SSH transport, queue, cancellation, expiry, and reconnect behavior.
...
```bash
bash scripts/install-can-workbench.sh /absolute/path/to/your-vault
```
...
1. Enable Can Workbench under Obsidian Community plugins.
2. Configure your own Local, Coder, or SSH workspace in Can Workbench settings.
3. Enable Secure Input and select the exact workspace.
4. Run `secenv doctor` in that workspace.
5. In Obsidian, run `Can Workbench: Secure input: Show listener status`; it must report `listening`.
...
If `secenv`, Can Workbench, its listener, or the native modal is unavailable, stop credential-dependent work and repair this path. Do not use chat, a shell prompt, a terminal paste, a browser link, an issue, a note, a screenshot, or a temporary “one-time” exception.
...
- Can Workbench v2.2.0 is desktop-only. Mobile Obsidian is not a secure-input surface.
- v2.2.0 does not mask `textarea`; the bundled `secenv` rejects secret-looking field names assigned to visible `text/textarea`. Multiline secrets require a future reviewed masked/file-input extension, not a fallback channel.

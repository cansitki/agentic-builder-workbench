...
async function openTerminalLink(app, rawUrl) {
  ...
  if (surfing && /^https?:\/\//i.test(url)) {
    try {
      ...
      console.warn("[Workbench] Could not open terminal link in Surfing", error);
    }
  }
...
var TerminalSession = class {
  ...
  installNormalSelectionMode() {
    const apply = () => {
      ...
      if (!selectionService || selectionService.__workbenchNormalSelection) return;
      ...
      selectionService.__workbenchNormalSelection = true;
    };
    apply();
  ...
  installVisibilityRenderGuard() {
    ...
    if (!selectionService || !renderService || selectionService.__workbenchVisibilityGuard) return;
    ...
    selectionService.refresh = (...args) => {
      ...
      selectionService.__workbenchDeferredRefresh = false;
    ...
    selectionService.__workbenchVisibilityGuard = true;
  }
  captureOutput() {
  ...
  show(skipFocus = false) {
    ...
    requestAnimationFrame(() => {
      this.fit();
      const selectionService = this.terminal?._core?._selectionService;
      if (selectionService?.__workbenchDeferredRefresh) selectionService.refresh();
...
var TerminalView = class extends import_obsidian.ItemView {
  ...
  getWorkbenchPlugin() {
    return this.app?.plugins?.plugins?.["workbench"] || null;
  }
  getSharedCategories() {

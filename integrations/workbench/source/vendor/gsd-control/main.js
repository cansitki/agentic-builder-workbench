...
var GSDSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  renderWorkspace(containerEl, ws, wsIdx) {
    ...
    new import_obsidian.Setting(wsSection).setName("SSH authentication").setDesc(
      "Use your existing SSH configuration or a key file path in the Workbench connection settings. New secrets are accepted only through secenv ask and the native secure-input modal."
    );
  ...
  async onOpen() {
    ...
    if (!wsConfig || !coderUser) {
      container.createDiv({
        ...
        text: "No workspace configured. Open Settings \u2192 Workbench to add a workspace and set your Coder username."
      });
      return;
    }
    if (!this.workspaceName) {
...
var ExplorerView = class extends import_obsidian4.ItemView {
  ...
  async fetchListing() {
    if (!this.selectedWorkspace) {
      this.error = "No workspace configured. Open Settings \u2192 Workbench.";
    ...
    try {
      ...
      if (!this.currentPath.startsWith("/")) {
        const remoteHome = (await execInWorkspace(host, 'printf "%s" "$HOME"')).trim();
        if (!remoteHome.startsWith("/") || /[\x00-\x1f\x7f]/.test(remoteHome)) throw new Error("Remote home could not be resolved");
        const relative = this.currentPath === "~" ? "" : this.currentPath.replace(/^~\//, "");
        this.currentPath = remoteHome.replace(/\/$/, "") + (relative ? "/" + relative : "");
      }
...
var TerminalPicker = class extends import_obsidian5.Modal {
  ...
  getLocalTmuxSessions(workspaceName = "__cw_local_tmux__") {
    try {
      ...
      const raw = execSync("tmux list-sessions -F '#{session_name}|||#{session_attached}|||#{@workbench_scope}' 2>/dev/null || true", { timeout: 5000 }).toString();
      const sessions = [];
  ...
  getSystemSessionInfo(sessionName, scope = "") {
    ...
    if (normalizedScope === "system") {
      return { label: "system", detail: "tmux @workbench_scope=system" };
    }
    const exact = new Map([
  ...
  async getSessionsForWorkspace(workspaceName, options = {}) {
    ...
    try {
      const raw = await execInWorkspace(host, "tmux list-sessions -F '#{session_name}|||#{session_attached}|||#{@workbench_scope}' 2>/dev/null || true", keyPath);
      const sessions = [];
      for (const line of raw.split("\n")) {
        if (!line.trim()) continue;
        const [name, attached, scope] = line.split("|||");
...
var GSDPlugin = class extends import_obsidian6.Plugin {
  ...
  async onload() {
    console.log("[Workbench] Loading Workbench plugin");
    ...
    console.log("[Workbench] Workbench plugin loaded");
  }
  async onunload() {
